#!/usr/bin/env python3
"""
抓取诺贝尔物理学奖得主的完整 Wikipedia 页面（复用数学家侧 fetch_full_pages.py 的下载方式）。
...
数据来源：presentations/Nobel_Physics_Laureates_20th_21st_Century.md
...
用法：
  ...
  python3 fetch_nobel_pages.py --only Einstein    # 只抓名字含 Einstein 的
...
USER_AGENT = (
    "OpenMathAI-PhysicistFetcher/1.0 "
    "(https://github.com/OpenMathAI/OpenMath; educational use) python-requests"
)

ROOT = Path(__file__).resolve().parent
LIST_MD = ROOT / "presentations" / "Nobel_Physics_Laureates_20th_21st_Century.md"
...
def main() -> int:
    ...
    print(f"将抓取 {len(laureates)} 位物理学家 → {OUT_ROOT.resolve()}")
