...
4. **品牌口径统一（共享 GitHub）★**：物理学家与数学家共享同一 GitHub 仓库 `OpenMathAI/OpenMath`（`https://github.com/OpenMathAI/OpenMath`）。
   - 项目首页模板 `cover/openphysicist_page.tex` 底部共建行已写入该链接，**所有物理学家 deck 通过 `\input` 自动继承，切勿在子 deck 重复或改写链接**。
   - **结尾页（closingslide）底部品牌标注统一写 `OpenMathAI`**（对齐数学家侧 Hilbert deck 的结尾页），不得写子品牌名 `OpenPhysicist`——"OpenPhysicist"仅作为首页模板/子产品名使用，GitHub 组织与结尾页归属一律用 `OpenMathAI`。
   - 引号风格：正文/引述一律用半角直引号 `" "`（与 Hilbert deck 一致），不要用中文弯引号 `“ ”`。
   - 身份信息页主标题允许用 `-` 连接一句叙事（如 `尤金·维格纳 - 从布达佩斯到诺贝尔奖`），作为速览页的例外写法；其余普通章节主标题保持简洁（空格/冒号），不塞副标题。
...
08  以维格纳命名的成果 — Jordan–Wigner、Wigner 晶体、Wigner 拟概率、Wigner's friend
09  曼哈顿计划 — 爱因斯坦–西拉德信、铀→钚转化、反应堆设计
