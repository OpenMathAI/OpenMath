#!/usr/bin/env python3
"""
从 Wikipedia 抓取所有物理学家并输出到 Markdown 文档。

数据来源：
  1. 英文 Wikipedia 「List of physicists」及相关子页面
  2. 调用 MediaWiki API 的 parse 接口，获取列表页的结构化链接。
  3. 兜底使用 Category:Physicists 分类递归抓取。

用法：
  python3 fetch_physicists.py                    # 默认输出到 physicists.md
  python3 fetch_physicists.py -o out.md          # 自定义输出文件
  python3 fetch_physicists.py --with-summary     # 抓取每位物理学家的简介（慢）
  python3 fetch_physicists.py --lang zh          # 中文 Wikipedia（按分类抓取）
"""
...
USER_AGENT = (
    "PhysicistsFetcher/1.0 "
    "(https://example.org/contact; educational use) "
    "python-requests"
)

# 英文 Wikipedia 物理学家列表页 —— 主页面及按字母拆分的子页面
EN_LIST_PAGES = [
    "List of physicists",
]
...
EXTRA_EN_LIST_PAGES = [
    ...
    "List of experimental physicists",
    "List of plasma physicists",
    "List of Russian physicists",
    "List of women in physics",
]
...
_NON_PERSON_KEYWORDS = [
    ...
    re.compile(r"\bparticle\b", re.I),
    re.compile(r"\bwave\b", re.I),
    re.compile(r"\benergy\b", re.I),
    re.compile(r"\bradiation\b", re.I),
    re.compile(r"\bsymmetry\b", re.I),
    re.compile(r"\binteraction\b", re.I),
    re.compile(r"\bscattering\b", re.I),
    re.compile(r"\boscillation\b", re.I),
    re.compile(r"\binstability\b", re.I),
    ...
    re.compile(r"\btelescope\b", re.I),
    re.compile(r"\bmicroscope\b", re.I),
    ...
    re.compile(r"\bcosmology\b", re.I),
    re.compile(r"\btechnolog\b", re.I),
    re.compile(r"\bengineer\b", re.I),
    re.compile(r"\bpinch\b", re.I),     # e.g. Z-pinch
]
...
def looks_like_person(title: str) -> bool:
    ...
    for pat in _BAD_TITLE_PATTERNS:
        if pat.search(title):
            return False
    # 过滤掉纯主题页（如 "Physics", "Quantum mechanics"）——极少会误伤人物
    if title.islower():
        return False
    # 排除明确为物理概念的条目
...
def collect_english(session: requests.Session) -> tuple[set[str], str]:
    ...
    for idx, page in enumerate(pages, 1):
        ...
        time.sleep(0.3)
    return all_titles, "英文维基百科「List of physicists」及相关列表页"
...
def main() -> int:
    ...
    ap.add_argument(
        "-o",
        "--output",
        default="physicists.md",
        help="输出的 Markdown 路径（默认 physicists.md）",
    )
    ap.add_argument(
        "--lang",
        default="en",
    ...
    ap.add_argument(
        "--category",
        default=None,
        help="改用分类递归方式抓取，例如 'Category:Physicists'（zh 默认使用 'Category:物理学家'）",
    )
    ap.add_argument(
        "--with-summary",
        action="store_true",
        help="同时抓取每位物理学家的简介（较慢）",
    )
    ap.add_argument(
        "--limit",
        type=int,
    ...
    if args.lang == "en" and not args.category:
        ...
        if not category:
            category = "Category:物理学家" if args.lang == "zh" else "Category:Physicists"
