#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""抓取并生成诺贝尔物理学奖得主清单 md 文档。

数据来源：英文维基百科「List of Nobel laureates in Physics」。
产出：presentations/Nobel_Physics_Laureates_20th_21st_Century.md
"""
...
WIKI_PAGE = 'https://en.wikipedia.org/wiki/List_of_Nobel_laureates_in_Physics'
OUT = Path(__file__).parent / 'presentations' / 'Nobel_Physics_Laureates_20th_21st_Century.md'
...
def write_md(laureates: list[dict]) -> None:
    ...
    lines = []
    lines.append('# 诺贝尔物理学奖得主（20 / 21 世纪）\n')
    lines.append('> 本清单收录 1901–2025 年诺贝尔物理学奖得主（共 %d 位）。\n' % len(laureates))
    lines.append('> 数据来源：英文维基百科「List of Nobel laureates in Physics」。\n')
