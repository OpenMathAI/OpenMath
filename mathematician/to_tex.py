#!/usr/bin/env python3
r"""
把 pages/<Name>/ 下的抓取数据转成一个可编译的 LaTeX 项目：
  tex/
    common/preamble.tex
    <Name>/
...
TOP_MAKEFILE_HEAD = r"""# Top-level Makefile for all mathematicians.
# Auto-generated by to_tex.py.

SUBDIRS := {subdirs}

.PHONY: all build clean distclean parallel {targets}

# 默认目标：编译每位的 PDF 后立即清理中间产物
all:
	@for d in $(SUBDIRS); do \
	  echo "==> $$d"; \
	  $(MAKE) -C $$d pdf || exit $$?; \
...
# 并行编译（-jN，N 为 CPU 核心数），同样会自动清理中间产物
parallel:
...
# 主流程
# ---------------------------------------------------------------------------
def convert_one(person_dir: Path, out_root: Path, download_imgs: bool) -> str | None:
...
def write_top_level(subdirs: list[str], out_root: Path) -> None:
    targets = " ".join(subdirs)
    content = TOP_MAKEFILE_HEAD.format(
    ...
    idx = [
        ...
        r"\title{Mathematicians Collection}",
        r"\author{}",
        r"\date{\today}",
    ...
    for d in subdirs:
        name = d.replace("_", " ")
        idx.append(rf"  \item \href{{{d}/{d}.pdf}}{{{name}}}")
    idx += [r"\end{itemize}", r"\end{document}"]
    (out_root / "index.tex").write_text("\n".join(idx), encoding="utf-8")
...
def main() -> int:
    ...
    preamble_src = ROOT / "common" / "preamble.tex"
    if preamble_src.exists():
        (COMMON_DIR / "preamble.tex").write_text(
            preamble_src.read_text(encoding="utf-8"), encoding="utf-8"
        )

    dirs = sorted(p for p in args.pages.iterdir() if p.is_dir())
    if args.only:
        dirs = [d for d in dirs if args.only.lower() in d.name.lower()]
    if args.limit:
        dirs = dirs[: args.limit]

    print(f"将转换 {len(dirs)} 位 → {args.out}")

    done: list[str] = []
    for i, d in enumerate(dirs, 1):
        print(f"\n[{i}/{len(dirs)}] {d.name}")
        try:
            name = convert_one(d, args.out, download_imgs=not args.no_images)
            if name:
                done.append(name)
    ...
    write_top_level(done, args.out)
    print(f"\n✅ 转换完成 {len(done)} 位。运行 `cd tex && make parallel` 开始编译。")
    return 0
