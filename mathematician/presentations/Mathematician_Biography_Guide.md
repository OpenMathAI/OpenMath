...
| 标志位 | 含义 | 取值 |
|--------|------|------|
| **立传** | 是否已完成演讲文稿制作 | `✅` = `.tex` 已完成并编译通过；`✅✅` = 已完成演讲+视频 |
| **Review** | 是否已完成 Review | `🔲` = 未开始；`✅` = 完成第1轮；`✅✅` = 完成第2轮 |

### 19.2 标志位更新时机

1. **立传 → ✅**：当 `make pdf` 编译通过、无 Error（仅允许预存 overfull 告警）时，将对应行标记为 `✅`
2. **Review → ✅**（第1轮完成）：当按 §16 完成 Wikipedia 逐页校验 + 标签/globe/footer/text_width 修复后，标记为 `✅`
3. **Review → ✅✅**（第2轮完成）：当在历史 Review 基础上再次完成全量复核、修正所有 P0/P1 问题后，标记为 `✅✅`

### 19.3 进度统计

排名文件底部有自动进度摘要行，格式：
```
> **进度摘要**：立传 X/50 ✅ · Review 2轮 Y/50 · Review 1轮 Z/50 · Review 未开始 W/50
```

每次完成一轮 Review 后，需同步更新：
1. 对应数学家的标志位（`🔲` → `✅` → `✅✅`）
2. 进度摘要行的统计数字

### 19.4 Review 清单模板

每完成一轮 Review，commit message 格式：
```
review: [名字] #[序号] — [修改摘要]
```

修改内容标准清单（第一轮）：
- [ ] Wikipedia 逐页事实校验（生卒、导师、年份、国籍）
- [ ] 排名标签展开为三维度
- [ ] 封面添加 `🌐` 国籍徽章
- [ ] 闭幕 footer 标准化（去国籍、去奖项、保留标签格式）
