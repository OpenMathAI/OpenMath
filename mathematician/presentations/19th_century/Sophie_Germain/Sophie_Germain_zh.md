...
1. **封面有头像**：右上角肖像 + `draw=rosegold!50` 细边框 + 姓名小字注（采用用户指定的正确素描 `images/Sophie-Germain.jpeg`，标注"非生前肖像"）。
...
- **【最重要】无生前肖像**：Germain **没有任何生前画像或照片**，也没有同时代的容貌描述。现有"素描"是基于 Zacharie Astruc 的半身像（该半身像又基于死亡面具）绘制的。**封面与身份信息页头像最终采用用户指定的正确素描 `images/Sophie-Germain.jpeg`（Astruc 半身像素描），并明确标注"非生前肖像"**——切勿误用他人照片，也勿把死亡面具照片当真人肖像。
...
- [ ] 【无生前肖像】封面/身份信息页头像用 `images/Sophie-Germain.jpeg`（Astruc 半身像素描，标注"非生前肖像"），勿用死亡面具当真人像
...
- [ ] **头像**：无生前肖像，采用用户指定 `images/Sophie-Germain.jpeg`（Astruc 半身像素描，标注"非生前肖像"）
- [ ] **国籍**：封面顶部徽章明示法国
