#!/usr/bin/env python3
"""
Download all Abel Prize laureates' Wikipedia pages with images for offline browsing.

Output: Abel_Prize/pages/<Year>/<Laureate>/index.html  (+ images/)

Usage:
  python3 fetch_abel_prize.py                  # download all 29 laureates
  python3 fetch_abel_prize.py --rate 2.0       # set request interval
  python3 fetch_abel_prize.py --no-images      # HTML only, no images
  python3 fetch_abel_prize.py --start 10       # start from index 10
...
# ---------------------------------------------------------------------------
# Abel Prize laureates, 2003-2026, 29 laureates.
# ---------------------------------------------------------------------------
LAUREATES = [
    ...
    (2008, "Jacques Tits", ""),
    (2009, "Mikhail Gromov (mathematician)", ""),
    (2010, "John Tate (mathematician)", ""),
    (2011, "John Milnor", ""),
...
<p>Source: <a href="https://{lang}.wikipedia.org/wiki/{title}">Wikipedia</a> · Abel Prize laureate · Downloaded for offline reading</p>
</footer>
</body>
</html>"""
...
def generate_index() -> None:
    lines = [
        ...
        "<title>Abel Prize Laureates · Wikipedia Offline</title>",
        "<style>",
        ...
        "h1{font-size:2em;border-bottom:3px solid #002f6c;padding-bottom:0.3em;}",
        "h2{color:#002f6c;border-bottom:1px solid #ccc;margin-top:1.5em;}",
        "ul{line-height:2;list-style:none;padding-left:1em;}",
        "li::before{content:'⚛ ';color:#002f6c;}",
        ...
        "</style></head><body>",
        f"<h1>⚛ Abel Prize Laureates ({2003}-{2026})</h1>",
    ...
    if current_year is not None:
        lines.append("</ul>")
    lines.append("<footer><p>Source: Wikipedia · Abel Prize laureates · Offline archive</p></footer>")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download Abel Prize laureates' Wikipedia pages")
    ...
    print(f"  Abel Prize laureates: {total} pages", file=sys.stderr)
    ...
    # Also download the "Abel Prize" award page itself
    print("\n  Downloading Abel Prize award page ...", file=sys.stderr)
    download_entry(0, "Abel Prize", "award_page")
    # Move from pages/0/ to pages/_prize/
    src = OUT_DIR / "0" / "Abel Prize"
    dst = OUT_DIR / "_prize" / "Abel Prize"
    if src.exists():
        dst.parent.mkdir(parents=True, exist_ok=True)
        src.rename(dst)
        (OUT_DIR / "0").rmdir()
        print(f"  ✓ Abel Prize award page saved to {dst}", file=sys.stderr)
