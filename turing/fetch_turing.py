#!/usr/bin/env python3
"""
Download all Turing Award winners' Wikipedia pages (with images) for offline browsing.

Output: turing/pages/<Year>/<Winner Name>/index.html  (+ images/)

Usage:
  python3 fetch_turing.py             # download all 81 winners
  python3 fetch_turing.py --rate 2.0  # set request interval
  python3 fetch_turing.py --no-images # HTML only, no images
...
# ---------------------------------------------------------------------------
# Turing Award winners (1966-2025, 81 winners)
# ---------------------------------------------------------------------------
WINNERS = [
    # (Year, Wikipedia title, display note)
    (1966, "Alan J. Perlis", ""),
    (1967, "Maurice Wilkes", ""),
    ...
    (2025, "Charles H. Bennett (physicist)", ""),
...
<p>Source: <a href="https://{lang}.wikipedia.org/wiki/{title}">Wikipedia</a> · Turing Award winner · Downloaded for offline reading</p>
</footer>
</body>
</html>"""
...
def generate_index():
    lines = [
        ...
        "<title>Turing Award Winners · Wikipedia Offline</title>",
        "<style>",
        ...
        "</style></head><body>",
        f"<h1>🏅 Turing Award Winners ({1966}-{2025})</h1>",
    ...
    lines.append(
        "<footer><p>Source: Wikipedia · Turing Award winners · Offline archive</p></footer>"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Download Turing Award winners' Wikipedia pages")
    ...
    print(f"  Turing Award Winners: {total} pages", file=sys.stderr)
