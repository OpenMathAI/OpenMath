...
- **气质定位**：史诗/奠基（算法分析之父、TAOCP 作者、TeX 创造者，改写计算机史）
- **选定曲目**：Alex-Productions《**New Lands**》（152k views，高受众/史诗/开阔）
- **落地文件**：`turing/presentations/Donald_Knuth/NewLands.wav`（已复制，不入 git）
- **选曲理由**：宏大开阔的史诗基调，匹配 TAOCP → 算法分析 → TeX → 文学编程的奠基叙事与结尾升华
