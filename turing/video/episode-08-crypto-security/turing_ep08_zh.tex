% Turing Award Video — Episode 8
% 密码学与量子信息（2000 – 2025）
% Beamer layout mirrors COPSS/Fields video series
% Source: turing/pages 离线页面 + turing_award_winners.md + Wikipedia
\documentclass[aspectratio=169,14pt]{beamer}
\usetheme{default}\usecolortheme{default}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{footline}{\hfill{\scriptsize\color{covermuted}\insertframenumber/\inserttotalframenumber}\hspace{0.4cm}\vspace{0.15cm}}
\usepackage{fontspec}\usepackage{xeCJK}
\setCJKmainfont{PingFang SC}[BoldFont=PingFang SC Semibold]
\setmainfont{Helvetica Neue}[BoldFont=Helvetica Neue Bold]
\usepackage{xcolor}\usepackage{tikz}\usepackage{graphicx}\usepackage{adjustbox}\usepackage{fontawesome5}\usepackage{ifthen}\usepackage{amssymb}
\usetikzlibrary{positioning,calc,arrows.meta,shadows}

% ===== 配色：图灵紫 + 琥珀金 + 青绿（区别于菲尔兹金/阿贝尔蓝/陈省身青/COPSS统计蓝）=====
\definecolor{bgmain}{RGB}{247,245,251}
\definecolor{coverprimary}{HTML}{5B2D8E}
\definecolor{coveraccent}{HTML}{B03A2E}
\definecolor{coveramber}{HTML}{D9A441}
\definecolor{coverpurple}{HTML}{1E8E8E}
\definecolor{coverdark}{HTML}{241A33}
\definecolor{covermuted}{HTML}{6B6478}
\definecolor{coverlight}{HTML}{EFEAF7}
\definecolor{titlecolor}{RGB}{36,26,51}
\definecolor{muteddark}{RGB}{96,90,110}
\definecolor{bluepanel}{RGB}{231,222,245}
\definecolor{redpanel}{RGB}{250,227,225}
\definecolor{goldpanel}{RGB}{250,243,226}
\definecolor{purplepanel}{RGB}{224,237,237}
\definecolor{graypanel}{RGB}{240,238,244}
\setbeamercolor{background canvas}{bg=bgmain}

\providecommand{\personhonors}{}

% 交叉奖项徽标（参考 Fields/COPSS 系列）
% 用 \raisebox 而非 \textsuperscript：避免 22pt 标题触发 15.4pt 数学字体缺失警告
% 符号+字母+品牌色：◆N 诺贝尔 · ★K 京都 · ◇G 哥德尔 · ★A 阿贝尔 · ★S 香农
\newcommand{\nobelbadge}{\raisebox{0.55ex}{\tiny\color{coveramber}$\clubsuit$\kern-0.5pt N}}
\newcommand{\kyotobadge}{\raisebox{0.55ex}{\tiny\color{coverprimary}$\blacklozenge$\kern-0.5pt K}}
\newcommand{\godelbadge}{\raisebox{0.55ex}{\tiny\color{coverpurple}$\diamond$\kern-0.5pt G}}
\newcommand{\abelbadge}{\raisebox{0.55ex}{\tiny\color{coveraccent}$\bigstar$\kern-0.5pt A}}
\newcommand{\shannonbadge}{\raisebox{0.55ex}{\tiny\color{coveramber}$\bigstar$\kern-0.5pt S}}
\newcommand{\hammingbadge}{\raisebox{0.55ex}{\tiny\color{coverpurple}$\bigstar$\kern-0.5pt H}}
\newcommand{\ntmbadge}{\raisebox{0.55ex}{\tiny\color{coverdark}$\star$\kern-0.5pt T}}
% ★W 沃尔夫 · ♥J 日本国际 · ▲N 内万林纳 · □E EATCS · ●M 马可尼 · ▲V 冯·诺依曼 · ◆T 千禧科技
\newcommand{\wolfbadge}{\raisebox{0.55ex}{\tiny\color{coveramber!70!black}$\bigstar$\kern-0.5pt W}}
\newcommand{\japanbadge}{\raisebox{0.55ex}{\tiny\color{coveraccent}$\heartsuit$\kern-0.5pt J}}
\newcommand{\nevanlinnabadge}{\raisebox{0.55ex}{\tiny\color{coverpurple}$\blacktriangle$\kern-0.5pt N}}
\newcommand{\eatcsbadge}{\raisebox{0.55ex}{\tiny\color{coverdark}$\square$\kern-0.5pt E}}
\newcommand{\marconibadge}{\raisebox{0.55ex}{\tiny\color{coveramber}$\bullet$\kern-0.5pt M}}
\newcommand{\neumannbadge}{\raisebox{0.55ex}{\tiny\color{coverprimary}$\blacktriangle$\kern-0.5pt V}}
\newcommand{\millenniumbadge}{\raisebox{0.55ex}{\tiny\color{coveraccent}$\blacklozenge$\kern-0.5pt T}}
% ♠M 国家科学奖章 · ■N 美国科学院 · ✓R 皇家学会 · ◆C 加拿大总督 · ◇R 鲁梅哈特
\newcommand{\nsmbadge}{\raisebox{0.55ex}{\tiny\color{coverprimary}$\spadesuit$\kern-0.5pt M}}
\newcommand{\nasbadge}{\raisebox{0.55ex}{\tiny\color{coverpurple}$\blacksquare$\kern-0.5pt N}}
\newcommand{\frsbadge}{\raisebox{0.55ex}{\tiny\color{coveramber}$\checkmark$\kern-0.5pt R}}
\newcommand{\govbadge}{\raisebox{0.55ex}{\tiny\color{coverdark}$\blacklozenge$\kern-0.5pt C}}
\newcommand{\rumelhartbadge}{\raisebox{0.55ex}{\tiny\color{coverpurple}$\diamondsuit$\kern-0.5pt R}}

\newcommand{\plainbar}{%
\begin{tikzpicture}[remember picture, overlay]
  \fill[coverdark, opacity=0.06] (current page.south west) rectangle ([yshift=0.4cm]current page.south east);
  \draw[coverprimary, opacity=0.40, line width=0.8pt] ([yshift=0.4cm]current page.south west) -- ([yshift=0.4cm]current page.south east);
\end{tikzpicture}%
}

% 底部交叉荣誉边框：#1 = 图标串（\godelbadge 哥德尔奖 2009 ...）
\newcommand{\honorbar}[1]{%
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south, draw=coverprimary!55, fill=coverlight, rounded corners=6pt,
          inner xsep=8pt, inner ysep=5pt, yshift=0.65cm, text=coverdark!85] at (current page.south) {%
      \fontsize{7.5}{9.5}\selectfont\bfseries{\color{coverprimary} 其他殊荣}\enspace #1};
  \end{tikzpicture}%
}

\newcommand{\sectiontitle}[2]{%
\vspace{-0.52cm}
\begin{center}
  {\fontsize{20}{24}\selectfont\bfseries\color{titlecolor} #1}\\[2pt]
  {\fontsize{7.5}{9.5}\selectfont\itshape\color{muteddark} #2}
\end{center}
\vspace{0.06cm}
}

% ---- Person slide: #1 name #2 subtitle #3 img/none #4 credit/缩写 #5 获奖(年+岁) #6 life #7 country #8 inst #9 contribution ----
\newcommand{\personslide}[9]{%
\begin{frame}
\plainbar
\vspace{-0.45cm}
\begin{center}
  {\fontsize{22}{26}\selectfont\bfseries\color{coverdark} #1}\\[2pt]
  {\fontsize{9.5}{11.5}\selectfont\color{muteddark} #2}
\end{center}
\vspace{0.1cm}
\begin{columns}[c]
\begin{column}{0.20\textwidth}
  \centering
  \ifthenelse{\equal{#3}{none}}{%
    \begin{tikzpicture}
      \node[circle, fill=coverprimary!12, draw=coverprimary!38, minimum size=2.6cm]
        {\fontsize{16}{19}\selectfont\bfseries\color{coverprimary!75!black} #4};
    \end{tikzpicture}
    {\par\vspace{1pt}\fontsize{6.2}{7.2}\selectfont\color{covermuted} 暂无公开照片\par}
  }{%
    \begin{tikzpicture}
      \node[draw=coverprimary!28, line width=0.7pt, fill=white, rounded corners=3pt, inner sep=2.5pt, drop shadow={shadow xshift=0.5pt, shadow yshift=-0.5pt, opacity=0.12}] {
        \includegraphics[width=2.2cm,height=2.7cm,keepaspectratio]{#3}
      };
    \end{tikzpicture}
    {\par\vspace{1pt}\fontsize{6.2}{7.2}\selectfont\color{covermuted} 图片来源：#4\par}
  }
\end{column}
\begin{column}{0.40\textwidth}
  \begin{tikzpicture}
    \node[fill=goldpanel, rounded corners=4pt, inner xsep=6pt, inner ysep=5pt, text width=4.0cm, align=left] {
      {\fontsize{7}{8.5}\selectfont\bfseries\color{coverprimary!70!black} 获奖}\enspace{\fontsize{7}{8.5}\selectfont\color{coverdark!85} #5}\\[2.5pt]
      {\fontsize{7}{8.5}\selectfont\bfseries\color{coverprimary!70!black} 生卒}\enspace{\fontsize{7}{8.5}\selectfont\color{coverdark!85} #6}\\[2.5pt]
      {\fontsize{7}{8.5}\selectfont\bfseries\color{coverprimary!70!black} 国别}\enspace{\fontsize{7}{8.5}\selectfont\color{coverdark!85} #7}\\[2.5pt]
      {\fontsize{7}{8.5}\selectfont\bfseries\color{coverprimary!70!black} 机构}\enspace{\fontsize{7}{8.5}\selectfont\color{coverdark!85} #8}
    };
  \end{tikzpicture}
\end{column}
\begin{column}{0.38\textwidth}
  \begin{tikzpicture}
    \node[draw=coverprimary!35, fill=bluepanel, rounded corners=5pt, inner sep=7pt, text width=3.8cm, align=left] {
      {\fontsize{8.2}{10}\selectfont\bfseries\color{coverprimary!82!black} 核心贡献}\\[3pt]
      {\fontsize{7}{8.5}\selectfont\color{coverdark!84} #9}
    };
  \end{tikzpicture}
\end{column}
\end{columns}
\if\relax\detokenize\expandafter{\personhonors}\relax\else\honorbar{\personhonors}\fi
\end{frame}
}

\newcommand{\coverslide}{%
\begin{frame}[plain]
\begin{tikzpicture}[remember picture, overlay]
  \fill[coverprimary!6] (current page.north west) rectangle (current page.south east);
  \fill[coveraccent!10] (current page.south east) ++(-2.3,-1.7) circle (2.0cm);
  \fill[coveramber!14] (current page.north east) ++(-2.4,1.4) circle (1.2cm);
  \fill[coverpurple!10] (current page.north west) ++(3.0,-1.0) circle (0.9cm);
  \node[anchor=center, font=\fontsize{17}{22}\selectfont\bfseries, text=coverdark]
    at ([yshift=2.60cm]current page.center) {ACM 图灵奖 · 密码学与量子信息（2000–2025）};
  \node[anchor=center, font=\fontsize{12}{15}\selectfont, text=coverprimary!85!black]
    at ([yshift=1.55cm]current page.center) {ACM Turing Award · 10 位得主};
  \draw[coverprimary, line width=1.4pt] ([yshift=0.75cm, xshift=-5.2cm]current page.center)
    -- ([yshift=0.75cm, xshift=5.2cm]current page.center);
  \node[anchor=center, font=\fontsize{9.5}{12.5}\selectfont\bfseries, text=coveramber!58!black]
    at ([yshift=0.10cm]current page.center) {公钥密码 · 零知识 · 量子密钥 · 从 Yao 到 Brassard};
  \node[anchor=center] at ([yshift=-0.30cm]current page.center) {
    \begin{tikzpicture}[scale=1]
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-2.98,0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/andrewyao.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-1.49,0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/ronaldrivest.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (0.00,0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/adishamir.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (1.49,0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/leonardadleman.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (2.98,0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/shafigoldwasser.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-2.98,-0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/silviomicali.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-1.49,-0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/whitfielddiffie.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (0.00,-0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/martinhellman.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (1.49,-0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/charleshbennett.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (2.98,-0.90) {\includegraphics[width=1.35cm,height=1.65cm,keepaspectratio]{images/gillesbrassard.jpg}};
    \end{tikzpicture}
  };
  \node[anchor=south, font=\scriptsize, text=coverdark!40]
    at ([yshift=0.38cm]current page.south) {\faIcon{medal}\enspace ACM Turing Award\enspace|\enspace 2000–2025 · 10 位得主};
\end{tikzpicture}
\end{frame}
}

% ========== SLIDES ==========

\newcommand{\andrewyaoslide}{\gdef\personhonors{\kyotobadge 京都奖 2021}\personslide
  {Andrew Yao\kyotobadge}{姚期智}
  {images/andrewyao.jpg}{Wikipedia}
  {2000（54岁）}{1946–}{美国/中国}{Princeton University / Tsinghua}
  {通信复杂性、密码学、量子计算——首位华人图灵奖得主。\\\textbf{主要荣誉}\\京都奖 2021}}

\newcommand{\ronaldrivestslide}{\gdef\personhonors{\marconibadge 马可尼奖 2007}\personslide
  {Ronald Rivest\marconibadge}{罗纳德·里维斯特}
  {images/ronaldrivest.jpg}{Wikipedia}
  {2002（55岁）}{1947–}{美国}{MIT}
  {RSA 公钥密码学三杰之一。\\\textbf{主要荣誉}\\马可尼奖 2007}}

\newcommand{\adishamirslide}{\gdef\personhonors{\wolfbadge 沃尔夫奖 2024 \enspace \frsbadge 皇家学会院士 2018}\personslide
  {Adi Shamir\wolfbadge\frsbadge}{阿迪·沙米尔}
  {images/adishamir.jpg}{Wikipedia}
  {2002（50岁）}{1952–}{以色列}{Weizmann Institute}
  {RSA 三杰之一，Shamir 秘密共享、密码分析。\\\textbf{主要荣誉}\\沃尔夫数学奖 2024 \\ 英国皇家学会院士 2018}}

\newcommand{\leonardadlemanslide}{\gdef\personhonors{}\personslide
  {Leonard Adleman}{伦纳德·阿德尔曼}
  {images/leonardadleman.jpg}{Wikipedia}
  {2002（57岁）}{1945–}{美国}{MIT}
  {RSA 三杰之一，DNA 计算先驱。}}

\newcommand{\shafigoldwasserslide}{\gdef\personhonors{\godelbadge 哥德尔奖 1993/2001 \enspace \nasbadge 美国科学院院士}\personslide
  {Shafi Goldwasser\godelbadge\nasbadge}{沙菲·戈德瓦瑟}
  {images/shafigoldwasser.jpg}{Wikipedia}
  {2012（54岁）}{1958–}{美国/以色列}{MIT / Weizmann}
  {概率加密、零知识证明，现代密码学奠基。\\\textbf{主要荣誉}\\哥德尔奖 1993/2001 \\ 美国科学院院士}}

\newcommand{\silviomicalislide}{\gdef\personhonors{\godelbadge 哥德尔奖 1993 \enspace \nasbadge 美国科学院院士}\personslide
  {Silvio Micali\godelbadge\nasbadge}{西尔维奥·米卡利}
  {images/silviomicali.jpg}{Wikipedia}
  {2012（58岁）}{1954–}{意大利/美国}{MIT}
  {零知识证明、可验证随机函数。\\\textbf{主要荣誉}\\哥德尔奖 1993 \\ 美国科学院院士}}

\newcommand{\whitfielddiffieslide}{\gdef\personhonors{\hammingbadge IEEE 汉明奖章 2010 \enspace \marconibadge 马可尼奖 2000 \enspace \frsbadge 皇家学会院士 2015}\personslide
  {Whitfield Diffie\hammingbadge\marconibadge\frsbadge}{惠特菲尔德·迪菲}
  {images/whitfielddiffie.jpg}{Wikipedia}
  {2015（71岁）}{1944–}{美国}{—}
  {公钥密码学（Diffie-Hellman 密钥交换）。\\\textbf{主要荣誉}\\马可尼奖 2000 \\ IEEE 汉明奖章 2010 \\ 英国皇家学会院士 2015}}

\newcommand{\martinhellmanslide}{\gdef\personhonors{\hammingbadge IEEE 汉明奖章 2010 \enspace \marconibadge 马可尼奖 2000}\personslide
  {Martin Hellman\hammingbadge\marconibadge}{马丁·赫尔曼}
  {images/martinhellman.jpg}{Wikipedia}
  {2015（70岁）}{1945–}{美国}{Stanford University}
  {公钥密码学（Diffie-Hellman 密钥交换）。\\\textbf{主要荣誉}\\马可尼奖 2000 \\ IEEE 汉明奖章 2010}}

\newcommand{\charleshbennettslide}{\gdef\personhonors{\wolfbadge 沃尔夫奖 2018 \enspace \shannonbadge 香农奖 2020}\personslide
  {Charles H. Bennett\wolfbadge\shannonbadge}{查尔斯·贝内特}
  {images/charleshbennett.jpg}{Wikipedia}
  {2025（82岁）}{1943–}{美国}{IBM}
  {量子信息科学、BB84 量子密钥分发、可逆计算。\\\textbf{主要荣誉}\\沃尔夫物理学奖 2018 \\ IEEE 香农奖 2020}}

\newcommand{\gillesbrassardslide}{\gdef\personhonors{\wolfbadge 沃尔夫奖 2018 \enspace \frsbadge 皇家学会院士 2013}\personslide
  {Gilles Brassard\wolfbadge\frsbadge}{吉勒·布拉萨}
  {images/gillesbrassard.jpg}{Wikipedia}
  {2025（70岁）}{1955–}{加拿大}{Université de Montréal}
  {量子密码学、BB84 协议、量子纠缠理论。\\\textbf{主要荣誉}\\沃尔夫物理学奖 2018 \\ 英国皇家学会院士 2013}}

\newcommand{\synthesisslide}{%
\begin{frame}
\plainbar
\sectiontitle{秘密与量子：密码学的新世纪}{密码学——从 RSA 到量子密钥分发}
\begin{center}
\begin{tikzpicture}[
  chip/.style={rounded corners=4pt, inner xsep=4pt, inner ysep=3pt, text width=1.9cm, align=center, font=\fontsize{6}{7.2}\selectfont}
]
  \node[chip, fill=bluepanel, draw=coverprimary!35, text=coverprimary!85!black] at (-5.0,1.75) {\textbf{2000}\\Yao};
  \node[chip, fill=goldpanel, draw=coveramber!35, text=coveramber!85!black] at (-2.5,1.75) {\textbf{2002}\\Rivest};
  \node[chip, fill=purplepanel, draw=coverpurple!35, text=coverpurple!85!black] at (0.0,1.75) {\textbf{2002}\\Shamir};
  \node[chip, fill=redpanel, draw=coveraccent!35, text=coveraccent!85!black] at (2.5,1.75) {\textbf{2002}\\Adleman};
  \node[chip, fill=bluepanel, draw=coverprimary!35, text=coverprimary!85!black] at (5.0,1.75) {\textbf{2012}\\Goldwasser};
  \node[chip, fill=goldpanel, draw=coveramber!35, text=coveramber!85!black] at (-5.0,0.25) {\textbf{2012}\\Micali};
  \node[chip, fill=purplepanel, draw=coverpurple!35, text=coverpurple!85!black] at (-2.5,0.25) {\textbf{2015}\\Diffie};
  \node[chip, fill=redpanel, draw=coveraccent!35, text=coveraccent!85!black] at (0.0,0.25) {\textbf{2015}\\Hellman};
  \node[chip, fill=bluepanel, draw=coverprimary!35, text=coverprimary!85!black] at (2.5,0.25) {\textbf{2025}\\Bennett};
  \node[chip, fill=goldpanel, draw=coveramber!35, text=coveramber!85!black] at (5.0,0.25) {\textbf{2025}\\Brassard};
\end{tikzpicture}
\end{center}
\vspace{0.12cm}
\begin{center}
\begin{tikzpicture}
  \node[draw=coverprimary!40, fill=bluepanel, rounded corners=5pt, inner sep=8pt, text width=13.4cm, align=center] at (0,0) {%
    {\fontsize{8}{9.5}\selectfont\bfseries\color{coverdark!80} 本集综述}\par\vspace{3pt}
    {\fontsize{7.4}{8.8}\selectfont\color{coverdark!72} RSA 与公钥密码守护互联网，零知识证明守护隐私，BB84 用量子物理守护未来。}
  };
\end{tikzpicture}
\end{center}
\end{frame}
}

% ========== MAIN ==========
\begin{document}

\coverslide

\andrewyaoslide
\ronaldrivestslide
\adishamirslide
\leonardadlemanslide
\shafigoldwasserslide
\silviomicalislide
\whitfielddiffieslide
\martinhellmanslide
\charleshbennettslide
\gillesbrassardslide

\synthesisslide

\end{document}
