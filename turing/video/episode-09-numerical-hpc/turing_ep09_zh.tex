% Turing Award Video — Episode 9
% 数值计算与高性能计算（1968 – 2021）
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=center, font=\fontsize{17}{22}\selectfont\bfseries, text=coverdark]
    at ([yshift=2.60cm]current page.center) {ACM 图灵奖 · 数值计算与高性能计算（1968–2021）};
  ...
  \node[anchor=center, font=\fontsize{9.5}{12.5}\selectfont\bfseries, text=coveramber!58!black]
    at ([yshift=0.10cm]current page.center) {浮点 · 纠错码 · HPC · 从 Hamming 到 Dongarra};
  \node[anchor=center] at ([yshift=-0.30cm]current page.center) {
    \begin{tikzpicture}[scale=1]
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-1.64,0.97) {\includegraphics[width=1.5cm,height=1.8cm,keepaspectratio]{images/richardhamming.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (0.00,0.97) {\includegraphics[width=1.5cm,height=1.8cm,keepaspectratio]{images/jameshwilkinson.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (1.64,0.97) {\includegraphics[width=1.5cm,height=1.8cm,keepaspectratio]{images/williamkahan.jpg}};
      \node[inner sep=0pt, draw=coverprimary!22, line width=0.2pt] at (-1.64,-0.97) {\includegraphics[width=1.5cm,height=1.8cm,keepaspectratio]{images/jackdongarra.jpg}};
    \end{tikzpicture}
  };
  \node[anchor=south, font=\scriptsize, text=coverdark!40]
    at ([yshift=0.38cm]current page.south) {\faIcon{medal}\enspace ACM Turing Award\enspace|\enspace 1968–2021 · 4 位得主};
\end{tikzpicture}
\end{frame}
}
...
\begin{frame}
\plainbar
\sectiontitle{数字的精度：数值与高性能计算}{数值计算——从汉明码到 LINPACK}
\begin{center}
\begin{tikzpicture}[
  ...
  \node[chip, fill=bluepanel, draw=coverprimary!35, text=coverprimary!85!black] at (-5.0,1.75) {\textbf{1968}\\Hamming};
  \node[chip, fill=goldpanel, draw=coveramber!35, text=coveramber!85!black] at (-2.5,1.75) {\textbf{1970}\\Wilkinson};
  \node[chip, fill=purplepanel, draw=coverpurple!35, text=coverpurple!85!black] at (0.0,1.75) {\textbf{1989}\\Kahan};
  \node[chip, fill=redpanel, draw=coveraccent!35, text=coveraccent!85!black] at (2.5,1.75) {\textbf{2021}\\Dongarra};
\end{tikzpicture}
\end{center}
\vspace{0.12cm}
\begin{center}
\begin{tikzpicture}
  \node[draw=coverprimary!40, fill=bluepanel, rounded corners=5pt, inner sep=8pt, text width=13.4cm, align=center] at (0,0) {%
    ...
    {\fontsize{7.4}{8.8}\selectfont\color{coverdark!72} 汉明码守护数字通信，IEEE 754 定义浮点世界，Dongarra 让超级计算机跑得更快。}
  };
\end{tikzpicture}
\end{center}
\end{frame}
