% =============================================================
%  OpenChemist Project Cover --- public template (for \input reuse)
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=center, font=\fontsize{24}{30}\selectfont\bfseries, text=coverprimary!90!black]
    at ([yshift=3.9cm]current page.center) {OpenChemist 已开放源码};
  \node[anchor=center, font=\fontsize{14}{18}\selectfont, text=coverprimary!60!black]
    at ([yshift=3.05cm]current page.center) {为 2,000+ 位化学家立传};
  ...
  \node[anchor=center, font=\fontsize{8.2}{11.5}\selectfont, text=coverdark!72, align=center, text width=13cm]
    ...
    {从拉瓦锡、道尔顿、门捷列夫，到居里夫人、鲍林、霍奇金，\\[2pt]
     OpenChemist 正在为化学家建立开放、持续演进的化学人物档案。};
  ...
  \node[anchor=north west, font=\fontsize{7}{9.2}\selectfont, text=coverdark!78, align=left]
    ...
    {\textcolor{badgequantum}{\faBook}\enspace\textbf{2,000+ 化学家} --- 持续收录、整理与补充\\[4pt]
     \textcolor{coveraccent}{\faMap}\enspace\textbf{人物生平} --- 教育经历、学术生涯、重要节点\\[4pt]
     \textcolor{badgesym}{\faTh}\enspace\textbf{化学贡献} --- 定理、理论、方法与历史影响};
  \node[anchor=north west, font=\fontsize{7}{9.2}\selectfont, text=coverdark!78, align=left]
    at ([xshift=0.3cm, yshift=-0.05cm]current page.center)
    {\textcolor{badgenuclear}{\faTrophy}\enspace\textbf{荣誉与传承} --- 诺贝尔奖、戴维奖章、普里斯特利奖章等\\[4pt]
     \textcolor{coverteal}{\faGlobe}\enspace\textbf{化学谱系} --- 学派、师承、合作与思想传承\\[4pt]
  ...
  \node[draw=coveraccent!50, fill=goldpanel, rounded corners=5pt, inner sep=9pt, text width=12.0cm, align=center]
    ...
    {{\fontsize{8.6}{11}\selectfont\bfseries\color{coverdark!88} 化学史，不应该只存在于少数人的书架上。}\\[2pt]
     {\fontsize{7.5}{9.5}\selectfont\color{covermuted} 让每一位化学家的故事都可以被发现、被理解、被连接。}};

  \node[anchor=south, font=\fontsize{7.8}{9.8}\selectfont\bfseries, text=coveraccent!80!black]
    at ([yshift=0.64cm]current page.south) {\faIcon{star}\enspace Star OpenChemist · 加入共建\enspace|\enspace https://github.com/OpenMathAI/OpenMath};
  \node[anchor=south, font=\fontsize{6.2}{7.8}\selectfont, text=covermuted]
    at ([yshift=0.38cm]current page.south) {欢迎 Star、Issue、PR，共同建设一部开放的化学家人物史。};
\end{tikzpicture}
\end{frame}
}
