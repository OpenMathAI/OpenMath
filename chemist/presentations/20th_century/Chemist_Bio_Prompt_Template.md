# 化学家立传提示词模板（OpenChemist · Beamer）

> 本文件是 OpenChemist 项目「20 世纪化学家立传」的**通用提示词模板**。标杆实例待定——物理学家侧以 Kenneth G. Wilson（1982 诺贝尔物理学奖，重整化群理论）为参照，化学家侧将在首个成品完成后沉淀。
...
- **目标项目**：OpenChemist —— 开放化学家人物史（与 OpenMath 数学家侧共享 GitHub `OpenMathAI/OpenMath`）。
- **标杆实例**：待定（首个化学家成品完成后，在此补充其目录路径，作为后续立传的可对照样板）。
- **单一产物**：每个化学家生成一份 Beamer deck（`.tex` → `.pdf`），外加一份人物专属提示词（`.md`）与必要的数据库脚本。
- **设计哲学**：化学家立传与数学家立传的核心差异在于——**化学家必须有「身份信息页」（Identity / Bio 速览页）**，且强调「研究领域」的结构化表达。这两点构成化学家模板的骨架，务必保留。

### 标准目录结构

```
chemist/presentations/20th_century/
├── Chemist_Bio_Prompt_Template.md      # 本模板（通用提示词 + tex 说明）
├── Nobel_Chemistry_Laureates_20th_21st_Century.md
├── cover/
│   └── openchemist_page.tex            # 项目首页模板（统一 \input）
├── {Benchmark_Chemist}/                 # 标杆实例（首个成品，待补充）
│   ├── {Benchmark_Chemist}.html          # 本地 Wikipedia
│   ├── {Benchmark_Chemist}_zh.tex        # Beamer 源码
│   ├── {Benchmark_Chemist}_zh.md         # 人物专属提示词
│   ├── Makefile                          # 编译/出图/合成视频
│   └── images/{Avatar}.jpg               # 肖像
└── {New_Chemist}/                      # 目标人物目录（按第 1 步创建）
    ├── {New_Chemist}.html
    ├── {New_Chemist}_zh.tex
    ├── {New_Chemist}_zh.md
    ├── Makefile
...
4. **品牌口径统一（共享 GitHub）**：结尾页底部品牌统一写 `OpenMathAI`（不是 `OpenChemist`）；GitHub 链接由首页模板 `\input` 继承，子 deck 不重复；引号用半角 `" "`。
...
- 在 `chemist/presentations/20th_century/` 下创建 `{Name}/` 与 `images/`。
...
> 以下骨架源自标杆实例（首个化学家成品的 `{Benchmark_Chemist}_zh.tex`，待补充）。
> 代码块内的 `<...>` 为需替换的人物专属占位符（如 `<AVATAR>` → `Curie`、`<NAME_ZH>` → `玛丽·居里`）。
...
[10] document 主体    —— 按序 \openchemistslide \titleslide ... \closingslide
```
...
```latex
% =============================================================
%  OpenChemist chemist biography template (Beamer)
...
- 主题金句 + 分隔线 + 致敬行 + 生卒年行。
- **底部品牌统一写 `OpenMathAI`**（不写 `OpenChemist`）。

---

## 五、背景音乐选择（★ 借鉴数学家指南 §15.7）

> **音乐库位置**：`/Users/ericksun/workspace/codebuddy/OpenMathAI/music_audio/`（化学家与数学家共享同一音乐库）。
> **精选曲目汇总**：`music_audio/curated_tracks.md`（★ 选曲前必读）。
> **工作机制**：每位化学家 Makefile 中有 `BGM = $(wildcard *.wav)`。将选定的 `.wav` 复制到该化学家子目录下，`make video` 自动检测并混入；BGM 略短于 slides 总时长时，ffmpeg `-shortest` 自动对齐。`.wav` 不入 git。
...
- 音乐气质应与化学家气质匹配（而非简单选"好听"的）。
- 优先从 `curated_tracks.md` 精选列表挑选。
- 不是所有化学家都需要 epic/heroic 风格。

### 5.3 按化学家气质推荐

| 化学家气质 | 推荐来源 | 推荐曲目示例 | 理由 |
|-----------|---------|------------|------|
| **史诗/奠基/革命**（Lavoisier, Mendeleev） | Alex-Productions | New Lands, Expedition | 宏大开阔，匹配"改写化学史"的气质 |
| **深沉/理性/尺度**（Pauling, Hodgkin） | Alex-Productions | Timeless, PAST | 内敛的深度，而非宏伟征服 |
| **悲剧/战争/反思**（Haber, Otto Hahn） | Inspiring Electronic | Lonesome, Through the Darkness | 暗色调，情感深沉 |
| **探索/远征**（van 't Hoff, Arrhenius） | Alex-Productions | Expedition, Eternals | 远征式叙事，理论探索 |
| **古典/庄严**（Berzelius, Dalton） | Beethoven · Karajan | Symphony No.3 "Eroica", No.9 | 古典音乐的庄严与永恒 |
| **现代/科技/计算**（Pople 计算化学, 跨界） | Inspiring Electronic | Falling Apart, Mirage | 电子质感，现代感 |
| **鼓舞/突破/年轻天才**（Woodward 式早熟, Curie） | Alex-Productions | Awaken, Daylight | 明亮轻快，天才之光 |

### 5.4 操作步骤

```bash
# 1. 打开 curated_tracks.md，根据化学家气质挑选曲目
# 2. 复制选定的 .wav 到该化学家子目录
cp /Users/ericksun/workspace/codebuddy/OpenMathAI/music_audio/alex-productions/42-SyPUvzEkPyc-Timeless.wav \
   /Users/ericksun/workspace/codebuddy/OpenMathAI/chemist/presentations/20th_century/{Name}/

# 3. make video 自动检测并使用
cd /Users/ericksun/workspace/codebuddy/OpenMathAI/chemist/presentations/20th_century/{Name}/
make video
```

> **实战案例**（化学家标杆，待补充）：选定契合该化学家气质的曲目（沉稳/纪录片/长期纲领），匹配其核心贡献的气质。
...
- 编辑 `chemist/generate_20th_century_list.py`，将姓名加入 `REVIEWS_DONE` 集合（与 `BIOGRAPHIES_DONE` 并列）：
  ```python
  REVIEWS_DONE = {
      "{Name}",
  }
...
- 重新生成名单：
  ```bash
  cd chemist && python3 generate_20th_century_list.py
  ```
...
| **"第一"断言** | "第一个提出 X 理论" | 化学史少有严格"第一"，多来源交叉验证 |
| **年份标注** | 标题标"理论 (1905)" | 写作年 ≠ 发表年，标题不标无法确认的年份 |
| **理论归属过度** | "独自发明 X" | 查清共同贡献者（如 Pauling 化学键理论有 Lewis 电子对思想源头） |
| **伪引语（最危险）** | 中文引号内"化学家原话" | 无原文不加引号，用间接引语 |
| **伪精确数字** | "已精确到小数点后 X 位" | 随时间变化的数字不固定 |
| **人物时间线** | "后来被 X 发展"但 X 其实更早 | 核实每个引用人物生卒年 |
| **现代语言包装历史人物** | 用"分子轨道""化学键"包装 19 世纪化学 | "今天我们称之为…""为后来…奠定基础" |
| **奖项归属** | "诺贝尔奖表彰 X" | 核对官方获奖理由原文（如 Pauling 是"化学键的本质"而非泛泛"化学键理论"） |
...
| 文件 | 用途 |
|------|------|
| `20th_century/{Benchmark_Chemist}/{Benchmark_Chemist}_zh.tex` | 标杆实例 Beamer 源码（待补充） |
| `20th_century/{Benchmark_Chemist}/{Benchmark_Chemist}_zh.md` | 标杆实例人物专属提示词（待补充） |
| `20th_century/{Benchmark_Chemist}/Makefile` | 编译/出图/视频 Makefile 模板（待补充） |
| `cover/openchemist_page.tex` | 项目首页模板（统一 `\input`） |
| `20th_century/{Benchmark_Chemist}/{Benchmark_Chemist}_zh.tex` | 化学家首例成品参考（待补充） |
