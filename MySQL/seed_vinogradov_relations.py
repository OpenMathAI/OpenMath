#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Ivan Vinogradov 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Ivan_Vinogradov/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Vinogradov）
    ("advisor-student", "J. V. Uspensky", "博士导师（圣彼得堡大学）"),
    # 同事/后继（无向）
    ("colleague", "Sergei Sobolev", "1941-46 年代理所长的院士"),
    ("colleague", "Yuri Linnik", "圣彼得堡数论学派后继"),
    ("colleague", "Hua Loo-Keng", "1946 访问苏联，三角和方法交流"),
]

MARKER = "[Vinogradov-材料待展开] "

FIELDS = [
    ("number theory", "数论", 0),
    ("analytic number theory", "解析数论", 1),
    ("algebraic number theory", "代数数论", 2),
]

AWARDS = [
    ("Stalin Prize", "斯大林奖", 1941),
    ("Lenin Prize", "列宁奖", 0),
    ("Hero of Socialist Labour", "社会主义劳动英雄", 0),
    ("Order of Lenin", "列宁勋章", 0),
    ("\"Hammer and Sickle\" gold medal", "锤子镰刀金质奖章", 0),
...
INSTITUTIONS = [
    ...
    ("Saint Petersburg State University", "employment", 1920, None),
    ("Perm State University", "employment", None, None),
    ("Tomsk State University", "employment", None, None),
    ("Steklov Institute of Mathematics", "employment", 1934, 1983),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "伊万·维诺格拉多夫",
            '["Ivan Matveevich Vinogradov","Виноградов","Vinogradov 方法","弱哥德巴赫猜想"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Vinogradov id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "J. V. Uspensky":
                f, t = pid, pid0  # 导师 → Vinogradov
            else:
                f, t = pid0, pid
        else:
            f, t = sorted([pid0, pid])
        ...
        cur.execute(
            ...
            "VALUES (%s,%s,%s,%s,'Vinogradov-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Ivan Vinogradov' OR b.name_en='Ivan Vinogradov'"""
    )
