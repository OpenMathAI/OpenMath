#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Mikio Sato（佐藤幹夫）的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Mikio_Sato/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Sato）
    ("advisor-student", "Shokichi Iyanaga", "博士导师（东京大学 1963）"),
    ("advisor-student", "Kōsaku Yosida", "导师（吉田耕作）"),
    # 学生（有向：Sato → 学生）
    ("advisor-student", "Masaki Kashiwara", "学生，D-模理论大师（阿贝尔奖 2025）"),
    ("advisor-student", "Michio Jimbo", "学生，量子群"),
    ("advisor-student", "Takahiro Kawai", "学生，微局部分析"),
    ("advisor-student", "Tetsuji Miwa", "学生，可积系统"),
    ("advisor-student", "Motohico Mulase", "学生，代数几何"),
    # 合作者（无向）
    ("collaborator", "John Tate", "Sato–Tate 猜想共同提出（独立）"),
    ("collaborator", "Alexander Grothendieck", "超函数与局部上同调（独立实现）"),
    ("colleague", "Pierre Schapira", "评价者（'把分析当代数几何处理'）"),
]

MARKER = "[Sato-材料待展开] "

FIELDS = [
    ("algebraic analysis", "代数分析", 0),
...
AWARDS = [
    ("Asahi Prize", "朝日奖", 1969),
    ("Japan Academy Prize", "日本学士院奖", 1976),
    ("Person of Cultural Merit", "文化功劳者", 1984),
    ("Rolf Schock Prize in Mathematics", "罗尔夫·朔克数学奖", 1997),
    ("Wolf Prize in Mathematics", "沃尔夫数学奖", 2003),
]

INSTITUTIONS = [
    ("University of Tokyo", "education", None, 1963),
    ("Osaka University", "employment", None, None),
    ("University of Tokyo", "employment", None, None),
    ("Kyoto University", "employment", 1970, 1991),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            '["Mikio Sato","Satō Mikio","佐藤 幹夫","代数分析","超函数"]',
            "Japanese mathematician, founder of algebraic analysis (1928–2023)",
            "1928-04-18",
            "2023-01-09",
            pid0,
        ),
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Sato id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name in ("Shokichi Iyanaga", "Kōsaku Yosida"):
                f, t = pid, pid0  # 导师 → Sato
            else:
                f, t = pid0, pid  # Sato → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Sato-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Sato Mikio' OR b.name_en='Sato Mikio'"""
    )
