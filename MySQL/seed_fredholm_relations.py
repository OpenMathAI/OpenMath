#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Erik Ivar Fredholm 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Erik_Ivar_Fredholm/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Fredholm）
    ("advisor-student", "Gösta Mittag-Leffler", "博士导师（乌普萨拉 1898）"),
    # 学生（有向：Fredholm → 学生）
    ("advisor-student", "Carl-Gustaf Rossby", "学生，气象学家"),
    ("advisor-student", "Nils Zeilon", "学生"),
    # 家族（配偶）
    ("spouse", "Agnes Maria Liljeblad", "妻（1911 结婚，Mittag-Leffler 前秘书）"),
    # 同事（无向）
    ("colleague", "Torsten Carleman", "同事，斯德哥尔摩数学界"),
]

MARKER = "[Fredholm-材料待展开] "

FIELDS = [
    ("integral equations", "积分方程", 0),
    ("operator theory", "算子理论", 1),
    ("functional analysis", "泛函分析", 2),
]

# 全部奖项收录
AWARDS = [
    ("Poncelet Prize", "庞斯莱奖", 1908),
    ("Björkén Prize", "比约肯奖", 1910),
]

INSTITUTIONS = [
    ("Royal Institute of Technology", "education", None, None),
    ("Uppsala University", "education", None, 1898),
    ("Stockholm University", "education", None, None),
    ("Stockholm University", "employment", 1898, 1927),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "埃里克·伊瓦尔·弗雷德霍姆",
            '["Ivar Fredholm","Fredholm 方程","Fredholm 行列式","积分方程理论"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Fredholm id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Gösta Mittag-Leffler":
                f, t = pid, pid0  # 导师 → Fredholm
            else:
                f, t = pid0, pid  # Fredholm → 学生
        elif rel == "spouse":
            f, t = pid0, pid
        else:
            f, t = sorted([pid0, pid])
        ...
        cur.execute(
            ...
            "VALUES (%s,%s,%s,%s,'Fredholm-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Erik Ivar Fredholm' OR b.name_en='Erik Ivar Fredholm'"""
    )
