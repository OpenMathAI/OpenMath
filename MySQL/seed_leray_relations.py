...
ME = "Jean Leray"
RELATIONS = [
    ("advisor-student", "Henri Villat", "博士导师（巴黎，1933）", "in"),
    ("advisor-student", "Armand Borel", "博士生，代数拓扑与李群", "out"),
    ("advisor-student", "István Fáry", "博士生，拓扑学", "out"),
    ("colleague", "Henri Cartan", "同事：Leray 谱序列 vs Cartan 方法（战后拓扑学双轨）", "undir"),
    ("colleague", "Jean-Pierre Serre", "同事：Serre 用 Leray 谱序列攻克同伦群", "undir"),
    ("colleague", "Laurent Schwartz", "同事（法国学派）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Leray')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
