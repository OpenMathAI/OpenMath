...
ME = "Heinz Hopf"
RELATIONS = [
    ("advisor-student", "Ludwig Bieberbach", "博士导师（柏林，1925）", "in"),
    ("advisor-student", "Erhard Schmidt", "导师之一（柏林），泛函分析", "in"),
    ("advisor-student", "Hans Freudenthal", "博士生，代数拓扑（Freudenthal 悬挂定理）", "out"),
    ("advisor-student", "Friedrich Hirzebruch", "博士生（ETH），代数几何与拓扑", "out"),
    ("advisor-student", "Michel Kervaire", "博士生，微分拓扑", "out"),
    ("colleague", "Pavel Alexandrov", "合作者：合著《Topologie》（1935 经典教科书）", "undir"),
    ("colleague", "Solomon Lefschetz", "同事（拓扑学同行）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Hopf')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
