#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""唐纳德·克努特 (Donald Knuth) 人物主记录 + 研究领域入库（对应提示词第 4 步）。

- 新建/更新 people 主记录：name_en='Donald Knuth'、name_zh='高德纳'、
...
- 国籍：United States（美国）
- 研究领域：4 大领域写入 fields + person_field（带 rank，算法分析最高）

数据来源：本地 Wikipedia (turing/pages/1974/Donald Knuth/index.html) infobox + 正文。
中文名「高德纳」为 Knuth 本人认可的中文名（1977 年由 Frances Yao 所起，见 Wikipedia 正文）。
"""
...
FIELDS = [
    ("analysis of algorithms", "算法分析", 0),
    ("computer typesetting", "计算机排版", 1),
    ("programming languages", "编程语言", 2),
    ("theoretical computer science", "理论计算机科学", 3),
]
...
def main():
    ...
    if row:
        ...
        cur.execute(
            ...
            "WHERE id=%s",
            (QID, "高德纳", "male", "1938-01-10", None,
             "American computer scientist and mathematician (1938–)", "computer scientist", pid),
        )
        ...
        cur.execute(
            ...
            (QID, NAME, "高德纳", "male", "1938-01-10", None,
             "American computer scientist and mathematician (1938–)", "computer scientist"),
        )
        pid = cur.lastrowid
