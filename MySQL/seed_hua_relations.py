#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Hua Luogeng（华罗庚）的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Hua_Luogeng/（Wikipedia 存档）
注意：库中主记录名 Hua Loo-Keng(89)，已立传 has_biography=1。
"""
import re
import unicodedata

from db_mysql import get_conn

NAME = "Hua Loo-Keng"
QID = "Q590111"

RELATIONS = [
    # 引路人/老师（有向：引路人 → Hua）
    ("advisor-student", "Xiong Qinglai", "伯乐，1931 年破格邀其入清华大学"),
    # 学术影响（无向）
    ("colleague", "G. H. Hardy", "剑桥圆法（Hardy–Littlewood）影响"),
    ("colleague", "Norbert Wiener", "1935-36 清华讲座（其向 Hardy 推荐华罗庚）"),
    ("colleague", "Jacques Hadamard", "1935-36 清华讲座"),
    ("colleague", "Ivan Vinogradov", "其三角和方法（华罗庚重新表述）"),
    # 学生（有向：Hua → 学生）
    ("advisor-student", "Chen Jingrun", "学生，陈氏定理（哥德巴赫猜想最佳结果）"),
    ("advisor-student", "Wang Yuan", "学生，数论（哥德巴赫/素数分布）"),
    ("advisor-student", "Pan Chengdong", "学生，数论"),
    ("advisor-student", "Ding Xiaqi", "学生，计算数学"),
    ("advisor-student", "Wan Zhexian", "学生，代数学"),
]

MARKER = "[Hua-材料待展开] "

FIELDS = [
    ("number theory", "数论", 0),
    ("algebra", "代数学", 1),
    ("complex analysis", "复分析", 2),
    ("matrix theory", "矩阵论", 3),
]

AWARDS = [
    ("Academician of the Chinese Academy of Sciences", "中国科学院院士", 1955),
    ("TWAS Fellow", "发展中国家科学院院士", 0),
    ("honorary doctor of the Chinese University of Hong Kong", "香港中文大学名誉博士", 0),
    ("honorary doctorate of the Henri Poincaré University", "南锡大学（亨利·庞加莱大学）名誉博士", 0),
    ("Foreign Associate of the National Academy of Sciences", "美国国家科学院外籍院士", 1982),
]

INSTITUTIONS = [
    ("Tsinghua University", "education", 1931, 1936),
    ("University of Cambridge", "education", 1936, 1938),
    ("Tsinghua University", "employment", 1938, 1946),
    ("Southwest Associated University", "employment", 1938, 1946),
    ("Institute for Advanced Study", "employment", 1946, 1948),
    ("University of Illinois Urbana-Champaign", "employment", 1948, 1950),
    ("Tsinghua University", "employment", 1950, None),
]


def norm(s: str) -> str:
    s = unicodedata.normalize("NFKD", s or "")
    s = "".join(c for c in s if not unicodedata.combining(c))
    return re.sub(r"[\s_\-\'\.\(\)·,]", "", s).lower()


def main():
    conn = get_conn()
    cur = conn.cursor()

    cur.execute("SELECT id, name_en, name_zh FROM people")
    people = [(pid, en, zh, norm(en or ""), norm(zh or "")) for pid, en, zh in cur.fetchall()]
    by_en = {ne: pid for pid, en, zh, ne, nz in people if ne}
    by_zh = {nz: pid for pid, en, zh, ne, nz in people if nz}

    cur.execute("SELECT id FROM occupations WHERE name_en='mathematician'")
    occ_id = cur.fetchone()[0]

    # ---------- 1. Hua 本人补齐 ----------
    cur.execute("SELECT id FROM people WHERE name_en=%s", (NAME,))
    row = cur.fetchone()
    if not row:
        cur.execute(
            "INSERT INTO people(name_en, primary_occupation, has_biography, qid) "
            "VALUES (%s,'mathematician',1,%s)",
            (NAME, QID),
        )
        pid0 = cur.lastrowid
    else:
        pid0 = row[0]
    cur.execute(
        "UPDATE people SET qid=%s, name_variants=%s, description=%s, "
        "birth_date=%s, death_date=%s, has_social_data=1 WHERE id=%s",
        (
            QID,
            '["Hua Luogeng","Hua Lo-keng","华罗庚","优选法","自学成才"]',
            "Chinese mathematician (1910–1985)",
            "1910-11-12",
            "1985-06-12",
            pid0,
        ),
    )
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Hua id={pid0} 已补齐 people 字段（has_social_data=1，has_biography 保持）")

    # ---------- 2. 研究领域 ----------
    for f_en, f_zh, rank in FIELDS:
        cur.execute("SELECT id FROM fields WHERE name_en=%s", (f_en,))
        frow = cur.fetchone()
        if not frow:
            cur.execute("INSERT INTO fields(name_en, name_zh) VALUES (%s,%s)", (f_en, f_zh))
            fid = cur.lastrowid
            print(f"  + 新建领域: {f_en} (id={fid})")
        else:
            fid = frow[0]
        cur.execute(
            "INSERT IGNORE INTO person_field(person_id, field_id, `rank`) VALUES (%s,%s,%s)",
            (pid0, fid, rank),
        )
    print("  领域关联完成")

    # ---------- 3. 奖项 ----------
    for a_en, a_zh, year in AWARDS:
        cur.execute("SELECT id FROM awards WHERE name_en=%s", (a_en,))
        arow = cur.fetchone()
        if not arow:
            cur.execute("INSERT INTO awards(name_en, name_zh, award_type) VALUES (%s,%s,'honor')", (a_en, a_zh))
            aid = cur.lastrowid
            print(f"  + 新建奖项: {a_en} (id={aid})")
        else:
            aid = arow[0]
        note = None
        if year == 0:
            note = "年份待查"
        cur.execute(
            "INSERT IGNORE INTO award_laureate(person_id, award_id, `year`, share_type, source, note) "
            "VALUES (%s,%s,%s,'独享','Wikipedia',%s)",
            (pid0, aid, year, note),
        )
    print("  奖项关联完成")

    # ---------- 4. 机构 ----------
    for inst, rel, sy, ey in INSTITUTIONS:
        cur.execute("SELECT id FROM institutions WHERE name_en=%s", (inst,))
        irow = cur.fetchone()
        if not irow:
            cur.execute("INSERT INTO institutions(name_en) VALUES (%s)", (inst,))
            iid = cur.lastrowid
            print(f"  + 新建机构: {inst} (id={iid})")
        else:
            iid = irow[0]
        cur.execute(
            "SELECT 1 FROM person_institution WHERE person_id=%s AND inst_id=%s AND relation=%s",
            (pid0, iid, rel),
        )
        if not cur.fetchone():
            cur.execute(
                "INSERT IGNORE INTO person_institution(person_id, inst_id, relation, start_year, end_year) "
                "VALUES (%s,%s,%s,%s,%s)",
                (pid0, iid, rel, sy, ey),
            )
    print("  机构关联完成")

    # ---------- 5. 国籍 ----------
    for idx, country in enumerate(("Republic of China", "People's Republic of China")):
        cur.execute("SELECT id FROM countries WHERE name_en=%s", (country,))
        cid = cur.fetchone()
        if not cid:
            cur.execute("INSERT IGNORE INTO countries(name_en, name_zh, is_current) VALUES (%s,%s,0)",
                        (country, "中华民国" if "Republic of China" == country else "中华人民共和国"))
            cid = cur.lastrowid
            print(f"  + 新建国家: {country} (id={cid})")
        else:
            cid = cid[0]
        cur.execute("INSERT IGNORE INTO person_nationality(person_id, country_id, `rank`) VALUES (%s,%s,%s)",
                    (pid0, cid, idx))
    print("  国籍关联完成")

    # ---------- 6. 社会关系 ----------
    created = 0
    relations_added = 0
    for rel, name, note in RELATIONS:
        pid = by_en.get(norm(name))
        if pid is None:
            pid = by_zh.get(norm(name))
        if pid is None:
            cur.execute(
                "INSERT INTO people(name_en, primary_occupation, has_biography) VALUES (%s,'mathematician',0)",
                (name,),
            )
            pid = cur.lastrowid
            cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                        (pid, occ_id))
            by_en[norm(name)] = pid
            created += 1
            print(f"  + 新建(占位): {name} (id={pid})")
        else:
            print(f"  已有: {name} (id={pid})")

        if rel == "advisor-student":
            if name == "Xiong Qinglai":
                f, t = pid, pid0  # 引路人 → Hua
            else:
                f, t = pid0, pid  # Hua → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Hua-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1

    conn.commit()
    print(f"\n新建人物: {created} · 新增关系: {relations_added}")
    cur.execute("SELECT COUNT(*) FROM people")
    print(f"people 总数: {cur.fetchone()[0]}")

    print("\n=== 校验：Hua 社会关系 ===")
    cur.execute(
        """SELECT a.name_en AS 甲, rt.name_zh AS 关系, b.name_en AS 乙, pr.note
        FROM person_relation pr
        JOIN people a ON a.id=pr.from_id
        JOIN people b ON b.id=pr.to_id
        JOIN relation_types rt ON rt.relation_key=pr.relation_type
        WHERE a.name_en='Hua Loo-Keng' OR b.name_en='Hua Loo-Keng'"""
    )
    for r in cur.fetchall():
        print(f"  {r[0]} {r[1]} {r[2]} — {(r[3] or '')[:40]}")

    conn.close()


if __name__ == "__main__":
    main()
