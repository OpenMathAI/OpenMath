#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Oscar Zariski 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Oscar Zariski"
RELATIONS = [
    ("advisor-student", "Guido Castelnuovo", "博士导师（罗马 Sapienza，1924）：经典意大利代数几何学派", "in"),
    ("advisor-student", "David Mumford", "博士生（哈佛），模空间理论", "out"),
    ("advisor-student", "Michael Artin", "博士生，代数几何", "out"),
    ("advisor-student", "Robin Hartshorne", "博士生，《代数几何》经典教材作者", "out"),
    ("advisor-student", "Heisuke Hironaka", "博士生，奇点消解（Fields 1970）", "out"),
    ("colleague", "André Weil", "同行：代数几何基础的重构（Zariski 拓扑 vs Weil 抽象代数几何）", "undir"),
    ("colleague", "Solomon Lefschetz", "同事（美国代数几何圈）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Zariski')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
