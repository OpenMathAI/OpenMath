#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Richard Bellman 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Richard_E._Bellman/（Wikipedia 存档）
...
RELATIONS = [
    # 导师（有向：导师 → Bellman）
    ("advisor-student", "Solomon Lefschetz", "博士导师（普林斯顿 1946）"),
    # 学生（有向：Bellman → 学生）
    ("advisor-student", "Christine Shoemaker", "学生，水资源/环境系统"),
    ("advisor-student", "John L. Casti", "学生，数学建模"),
    ("advisor-student", "Augustine Esogbue", "学生，系统工程"),
    ("advisor-student", "Kenneth L. Cooke", "学生，延迟微分方程"),
    ("advisor-student", "Edward Stanley Angel", "学生"),
]

MARKER = "[Bellman-材料待展开] "

FIELDS = [
    ("dynamic programming", "动态规划", 0),
    ("control theory", "控制理论", 1),
    ("applied mathematics", "应用数学", 2),
    ("mathematical biology", "生物数学", 3),
    ("optimization", "最优化", 4),
]

AWARDS = [
    ("John von Neumann Theory Prize", "约翰·冯·诺伊曼理论奖", 1976),
    ("IEEE Medal of Honor", "IEEE 荣誉奖章", 1979),
    ("Norbert Wiener Prize in Applied Mathematics", "诺伯特·维纳应用数学奖", 0),
    ("Dickson Prize in Science", "迪克森科学奖", 0),
    ("Fellow of the American Academy of Arts and Sciences", "美国艺术与科学院院士", 0),
    ("Richard E. Bellman Control Heritage Award", "贝尔曼控制遗产奖", 1984),
]

INSTITUTIONS = [
    ("Brooklyn College", "education", None, 1941),
    ("University of Wisconsin–Madison", "education", None, None),
    ("Princeton University", "education", None, 1946),
    ("Los Alamos National Laboratory", "employment", 1944, 1946),
    ("RAND Corporation", "employment", 1949, 1965),
    ("Stanford University", "employment", 1965, 1967),
    ("University of Southern California", "employment", 1967, 1984),
    ("Johns Hopkins University", "employment", None, None),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            '["Richard Ernest Bellman","动态规划之父","Bellman 方程","维数灾难"]',
            "American mathematician, father of dynamic programming (1920–1984)",
            "1920-08-26",
            "1984-03-19",
            pid0,
        ),
    ...
    if cs_id:
        cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,1)",
                    (pid0, cs_id[0]))
    print(f"Bellman id={pid0} 已补齐 people 字段（has_social_data=1，has_biography 保持）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Solomon Lefschetz":
                f, t = pid, pid0  # 导师 → Bellman
            else:
                f, t = pid0, pid  # Bellman → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Bellman-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Richard Bellman' OR b.name_en='Richard Bellman'"""
    )
