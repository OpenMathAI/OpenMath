#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Marston Morse 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Marston Morse"
RELATIONS = [
    ("advisor-student", "George David Birkhoff", "博士导师（哈佛，1917）", "in"),
    ("advisor-student", "Gustav Hedlund", "博士生，动力系统与符号动力学", "out"),
    ("advisor-student", "Arthur Sard", "博士生，Sard 定理", "out"),
    ("colleague", "Hassler Whitney", "同事（哈佛/IAS）：微分拓扑与几何分析的对话者", "undir"),
    ("colleague", "Oswald Veblen", "同事（IAS）：普林斯顿拓扑圈核心成员", "undir"),
    ("colleague", "Solomon Lefschetz", "同事（普林斯顿/IAS）：代数拓扑与微分拓扑之交", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Morse')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
