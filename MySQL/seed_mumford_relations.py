#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""David Mumford 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "David Mumford"
RELATIONS = [
    ("advisor-student", "Oscar Zariski", "博士导师（哈佛，1961）", "in"),
    ("advisor-student", "Amnon Neeman", "博士生，三角范畴", "out"),
    ("advisor-student", "Emma Previato", "博士生，代数曲线与可积系统", "out"),
    ("advisor-student", "Ching-Li Chai", "博士生，算术几何（Chai）", "out"),
    ("colleague", "Alexander Grothendieck", "同事：代数几何与概形理论的对话者", "undir"),
    ("collaborator", "Pierre Deligne", "合作者：Deligne–Mumford 叠（模曲线）", "undir"),
    ("colleague", "Michael Artin", "同事（同门 Zariski 学生）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Mumford')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
