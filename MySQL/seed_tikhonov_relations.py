#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Andrey Tikhonov 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Andrey_Nikolayevich_Tikhonov/（Wikipedia 存档）
注意：死亡日期 metadata 为 1993-11-07（正文 infobox 为 10-07，取 metadata）。
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Tikhonov）
    ("advisor-student", "Pavel Alexandrov", "博士导师（1927）"),
    # 学生（有向：Tikhonov → 学生）
    ("advisor-student", "Aleksandr Samarsky", "学生，计算数学"),
    ("advisor-student", "Alexei Sveshnikov", "学生，数学物理"),
    ("advisor-student", "Vladimir Ilyin", "学生，PDE/泛函分析"),
    ("advisor-student", "Andrei Krylov", "学生"),
    ("advisor-student", "Anatoly Yagola", "学生，不适定问题"),
]

MARKER = "[Tikhonov-材料待展开] "

FIELDS = [
    ("topology", "拓扑学", 0),
    ("functional analysis", "泛函分析", 1),
    ("mathematical physics", "数学物理", 2),
    ("inverse problem", "反问题", 3),
    ("geophysics", "地球物理学", 4),
]

AWARDS = [
    ("Lenin Prize", "列宁奖", 1966),
    ("Hero of Socialist Labour", "社会主义劳动英雄", 1954),
    ("Hero of Socialist Labour", "社会主义劳动英雄（第二次）", 1986),
    ("Order of Lenin", "列宁勋章", 0),
    ("Order of the October Revolution", "十月革命勋章", 0),
    ("Order of the Red Banner of Labour", "劳动红旗勋章", 0),
    ("State Stalin Prize, 1st degree", "斯大林一等奖", 0),
    ("USSR State Prize", "苏联国家奖", 0),
    ("Lomonosov Gold Medal", "罗蒙诺索夫金质奖章", 0),
    ("Keldysh Gold Medal", "凯尔迪什金质奖章", 0),
    ("VDNKh gold medal", "全苏展览中心金质奖章", 0),
]

INSTITUTIONS = [
    ("Lomonosov Moscow State University", "education", None, 1927),
    ("Lomonosov Moscow State University", "employment", 1933, 1993),
    ("Keldysh Institute of Applied Mathematics", "employment", None, None),
    ("Schmidt Institute of Physics of the Earth", "employment", None, None),
    ("National Research Nuclear University MEPhI", "employment", None, None),
    ("Hydrometeorological Centre of Russia", "employment", None, None),
]
...
def main():
    ...
    cur.execute("SELECT id FROM occupations WHERE name_en='geophysicist'")
    geo_id = cur.fetchone()
    ...
    cur.execute(
        ...
        (
            QID,
            "安德烈·吉洪诺夫",
            '["Andrey Nikolayevich Tikhonov","Tychonoff","Tichonov","Tihonov","Tikhonov 正则化"]',
            "Soviet mathematician and geophysicist (1906–1993)",
            "1906-10-17",
            "1993-11-07",
            pid0,
        ),
    ...
    if geo_id:
        cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,1)",
                    (pid0, geo_id[0]))
    print(f"Tikhonov id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    # ---------- 5. 国籍 ----------
    for idx, country in enumerate(("Russian Empire", "Soviet Union", "Russia")):
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Pavel Alexandrov":
                f, t = pid, pid0  # 导师 → Tikhonov
            else:
                f, t = pid0, pid  # Tikhonov → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Tikhonov-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Andrey Tikhonov' OR b.name_en='Andrey Tikhonov'"""
    )
