#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Torsten Carleman 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Torsten_Carleman/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Carleman）
    ("advisor-student", "Erik Albert Holmgren", "博士导师（乌普萨拉）"),
    # 学生（有向：Carleman → 学生）
    ("advisor-student", "Åke Pleijel", "学生，调和分析"),
    ("advisor-student", "Hans Rådström", "学生，凸分析"),
    # 相关（无向）
    ("colleague", "Gösta Mittag-Leffler", "Mittag-Leffler 研究所（其任所长 20+ 年）"),
    ("colleague", "Erik Ivar Fredholm", "瑞典分析学派传统"),
]

MARKER = "[Carleman-材料待展开] "

FIELDS = [
    ("mathematical analysis", "数学分析", 0),
    ("complex analysis", "复分析", 1),
    ("integral equations", "积分方程", 2),
    ("operator theory", "算子理论", 3),
]

AWARDS = [
    ("Björkén Prize", "比约肯奖", 1941),
    ("Cours Peccot", "佩科课程奖", 1922),
]

INSTITUTIONS = [
    ("Uppsala University", "education", None, None),
    ("Lund University", "employment", None, None),
    ("Stockholm University", "employment", None, None),
    ("Mittag-Leffler Institute", "employment", None, None),
]
...
def main():
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Carleman id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Erik Albert Holmgren":
                f, t = pid, pid0  # 导师 → Carleman
            else:
                f, t = pid0, pid  # Carleman → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Carleman-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Torsten Carleman' OR b.name_en='Torsten Carleman'"""
    )
