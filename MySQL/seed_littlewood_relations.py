#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""J.E. Littlewood 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "J.E. Littlewood"
RELATIONS = [
    ("advisor-student", "Ernest Barnes", "博士导师（剑桥），特殊函数", "in"),
    ("advisor-student", "Harold Davenport", "博士生，解析数论（Selberg/Davenport 学派）", "out"),
    ("advisor-student", "Sarvadaman Chowla", "博士生，数论", "out"),
    ("advisor-student", "Peter Swinnerton-Dyer", "博士生，椭圆曲线（BSD 猜想）", "out"),
    ("collaborator", "G. H. Hardy", "终身合作者：Hardy–Littlewood 圆法，35 年通信合作", "undir"),
    ("colleague", "Srinivasa Ramanujan", "同事（剑桥）：哈代-拉马努金-李特尔伍德三角", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Littlewood')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
