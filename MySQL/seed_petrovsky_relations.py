#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Ivan Petrovsky 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Ivan_Petrovsky/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    ...
    # 学生（有向：Petrovsky → 学生）
    ("advisor-student", "Olga Ladyzhenskaya", "学生，PDE/流体力学"),
    ("advisor-student", "Olga Oleinik", "学生，PDE/自由边界"),
    ("advisor-student", "Sergei Godunov", "学生，数值分析（Godunov 格式）"),
    ("advisor-student", "Aleksei Filippov", "学生，微分包含"),
    ("advisor-student", "Evgenii Landis", "学生，PDE/复分析"),
]

MARKER = "[Petrovsky-材料待展开] "

FIELDS = [
    ("partial differential equation", "偏微分方程", 0),
    ("algebraic geometry", "代数几何", 1),
    ("probability theory", "概率论", 2),
    ("boundary value problem", "边值问题", 3),
]

AWARDS = [
    ("Stalin Prize", "斯大林奖", 0),
    ("Hero of Socialist Labour", "社会主义劳动英雄", 1969),
    ("Order of Lenin", "列宁勋章", 0),
    ("Order of the Red Banner of Labour", "劳动红旗勋章", 0),
    ("Commander of the Legion of Honour", "荣誉军团指挥官", 0),
    ("Medal \"For Valiant Labour in the Great Patriotic War 1941–1945\"", "伟大卫国战争英勇劳动奖章", 0),
    ("Patriotic Order of Merit", "爱国功绩勋章", 0),
]

INSTITUTIONS = [
    ("Lomonosov Moscow State University", "education", None, None),
    ("Lomonosov Moscow State University", "employment", None, None),
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "伊万·彼得罗夫斯基",
            '["Ivan Georgiyevich Petrovsky","Петровский","Petrovsky 空隙","莫斯科大学校长"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Petrovsky id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Dmitri Egorov":
                f, t = pid, pid0  # 导师 → Petrovsky
            else:
                f, t = pid0, pid  # Petrovsky → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Petrovsky-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Ivan Petrovsky' OR b.name_en='Ivan Petrovsky'"""
    )
