#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Mikhail Suslin 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Mikhail_Suslin/（Wikipedia 存档）
注意：勿与 Andrei Suslin(549) 混淆。
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Suslin）
    ("advisor-student", "Nikolai Luzin", "博士导师（莫斯科大学 1917）"),
    # 相关（无向）
    ("colleague", "Henri Lebesgue", "1917 年发现其论证中的错误（投影不保 Borel 集）"),
]
...
INSTITUTIONS = [
    ("Lomonosov Moscow State University", "education", 1913, 1917),
    ("Ivanovo-Voznesensk Polytechnic Institute", "employment", 1917, 1919),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "米哈伊尔·苏斯林",
            '["Mikhail Yakovlevich Suslin","Souslin","Суслин","解析集理论","Suslin 问题"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Suslin id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        cur.execute(
            ...
            "VALUES (%s,%s,%s,%s,'Suslin-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Mikhail Suslin' OR b.name_en='Mikhail Suslin'"""
    )
