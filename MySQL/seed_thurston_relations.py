#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""William Thurston 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "William Thurston"
RELATIONS = [
    ("advisor-student", "Morris Hirsch", "博士导师（伯克利，1972），微分拓扑", "in"),
    ("advisor-student", "Oded Schramm", "博士生，随机几何与 SLE", "out"),
    ("advisor-student", "David Gabai", "博士生，三维流形几何", "out"),
    ("advisor-student", "Benson Farb", "博士生，几何群论", "out"),
    ("collaborator", "John Milnor", "合作者：Milnor–Thurston 揉搓理论（kneading theory）", "undir"),
    ("colleague", "Dennis Sullivan", "同事：几何化时代的盟友与思想对话者", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Thurston')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
