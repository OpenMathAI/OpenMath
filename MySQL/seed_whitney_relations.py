#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hassler Whitney 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Hassler Whitney"
RELATIONS = [
    ("advisor-student", "George David Birkhoff", "博士导师（哈佛，1932）；与 Morse 同导师", "in"),
    ("advisor-student", "Herbert Robbins", "博士生，统计学（Robbins 引理）", "out"),
    ("advisor-student", "James Eells", "博士生，调和映照", "out"),
    ("colleague", "Marston Morse", "同事（哈佛/IAS）：微分拓扑的并肩开拓者", "undir"),
    ("colleague", "Norman Steenrod", "同事：纤维丛理论，Whitney–Steenrod 特征类", "undir"),
    ("colleague", "Saunders Mac Lane", "同事（哈佛）：拓扑与范畴论的交流", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Whitney')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
