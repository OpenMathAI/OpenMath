#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Camille Jordan 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Camille_Jordan/（Wikipedia 存档）
注意：勿与 Wilhelm Jordan（大地测量）或 Pascual Jordan（物理学家）混淆。
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Jordan）
    ("advisor-student", "Victor Puiseux", "学术顾问（1860 论文）"),
    ("advisor-student", "Joseph Alfred Serret", "学术顾问（1860 论文）"),
    # 学生
    ("advisor-student", "Marie Georges Humbert", "学生"),
    # 学术影响（无向）
    ("colleague", "Émile Picard", "后继的法国数学家"),
    ("colleague", "Édouard Goursat", "后继的法国数学家"),
]
...
FIELDS = [
    ("group theory", "群论", 0),
    ("linear algebra", "线性代数", 1),
    ("measure theory", "测度论", 2),
    ("topology", "拓扑学", 3),
]

AWARDS = [
    ("Poncelet Prize", "庞斯莱奖", 1870),
    ("Officer of the Legion of Honour", "荣誉军团军官", 0),
    ("Foreign Member of the Royal Society", "英国皇家学会外籍院士", 0),
]

INSTITUTIONS = [
    ("École Polytechnique", "education", None, None),
    ("Mines ParisTech", "education", None, None),
    ("École Polytechnique", "employment", None, None),
    ("Collège de France", "employment", None, None),
    ("Corps des Ponts et Chaussées", "employment", None, None),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "卡米尔·若尔当",
            '["Marie Ennemond Camille Jordan","Jordan 曲线定理","Jordan 标准形","群论奠基人"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Jordan id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name in ("Victor Puiseux", "Joseph Alfred Serret"):
                f, t = pid, pid0  # 导师 → Jordan
            else:
                f, t = pid0, pid  # Jordan → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Jordan-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Camille Jordan' OR b.name_en='Camille Jordan'"""
    )
