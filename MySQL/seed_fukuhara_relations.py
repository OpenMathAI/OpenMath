#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 福原満洲雄 (Fukuhara Masuo) 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/福原満洲雄/（日文 Wikipedia 存档）
注意：生卒以正文 1905-12-24 / 2007-02-07 为准。
"""
import re
import unicodedata
...
RELATIONS = [
    # 家族（无向）
    ("parent-child", "福原鐐二郎", "父亲（三男），日本官界人物"),
    # 相关（无向）
    ("colleague", "吉田耕作", "《微分方程式论》编辑合作（Kōsaku Yosida）"),
    ("colleague", "山中健", "《变分学入门》合著者"),
    ("colleague", "正田建次郎", "译著解説者（Hadamard/Poincaré 系列）"),
]

MARKER = "[Fukuhara-材料待展开] "

FIELDS = [
    ("differential equation", "常微分方程", 0),
    ("complex differential equation", "复微分方程", 1),
    ("mathematical analysis", "数学分析", 2),
]

AWARDS = [
    ("Japan Academy Prize", "日本学士院奖", 1963),
]

INSTITUTIONS = [
    ("University of Tokyo", "education", None, 1929),
    ("Kyushu University", "employment", 1939, None),
    ("University of Tokyo", "professor", 1948, None),
    ("Kyoto University", "professor", 1963, None),
    ("Tokyo University of Agriculture and Technology", "employment", 1973, 1979),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            '["福原満洲雄","Fukuhara Masuo","RIMS 所长","常微分方程"]',
            "Japanese mathematician (1905–2007)",
            "1905-12-24",
            "2007-02-07",
            pid0,
        ),
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Fukuhara id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    # ---------- 5. 国籍 ----------
    for idx, country in enumerate(("Japan", "Empire of Japan")):
        ...
        if not cid:
            cur.execute("INSERT IGNORE INTO countries(name_en, name_zh, is_current) VALUES (%s,%s,0)",
                        (country, "大日本帝国" if "Empire" in country else "日本"))
            cid = cur.lastrowid
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "parent-child":
            f, t = pid, pid0  # 父 → Fukuhara
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Fukuhara-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Fukuhara Masuo' OR b.name_en='Fukuhara Masuo'"""
    )
