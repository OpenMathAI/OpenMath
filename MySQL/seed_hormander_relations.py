#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Lars Hörmander 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Lars Hörmander"
RELATIONS = [
    ("advisor-student", "Marcel Riesz", "博士导师（隆德，1955），泛函分析", "in"),
    ("advisor-student", "Lars Gårding", "导师之一（隆德），PDE", "in"),
    ("advisor-student", "Johannes Sjöstrand", "博士生，半经典分析与微局部分析", "out"),
    ("advisor-student", "Christer Kiselman", "博士生，多复变与凸分析", "out"),
    ("advisor-student", "Jan Boman", "博士生，层论与微局部分析", "out"),
    ("colleague", "Louis Nirenberg", "同事（PDE 巨头）：Hörmander–Nirenberg 交流", "undir"),
    ("colleague", "Israel Gelfand", "同事（IAS/泛函分析界）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Hormander')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
