#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Oskar Perron 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Oskar_Perron/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Perron）
    ("advisor-student", "Ferdinand von Lindemann", "博士导师（慕尼黑 1902）"),
    # 学生（有向：Perron → 学生）
    ("advisor-student", "Helmut Röhrl", "学生，代数几何"),
    ("advisor-student", "Georgi Bradistilov", "学生"),
    ("advisor-student", "Ernst Peschl", "学生，复分析"),
    ("advisor-student", "Kurt Vogel", "学生，数学史"),
]
...
AWARDS = [
    ("Bavarian Order of Merit", "巴伐利亚功绩勋章", 0),
    ("honorary doctor of the University of Tübingen", "蒂宾根大学名誉博士", 0),
    ("Iron Cross", "铁十字勋章", 0),
]

INSTITUTIONS = [
    ("Ludwig Maximilian University of Munich", "education", None, 1902),
    ("Heidelberg University", "employment", 1914, 1922),
    ("Ludwig Maximilian University of Munich", "professor", 1922, 1951),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "奥斯卡·佩龙",
            '["Oskar Perron","Perron–Frobenius 定理","Perron 方法","连分数"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Perron id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Ferdinand von Lindemann":
                f, t = pid, pid0  # 导师 → Perron
            else:
                f, t = pid0, pid  # Perron → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Perron-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Oskar Perron' OR b.name_en='Oskar Perron'"""
    )
