#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Harish-Chandra 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Harish-Chandra"
RELATIONS = [
    ("advisor-student", "Paul Dirac", "博士导师（剑桥，1947）：物理学家，量子力学奠基人之一", "in"),
    ("collaborator", "Armand Borel", "合作者：Borel–Harish-Chandra 定理（算术子群）", "undir"),
    ("colleague", "Robert Langlands", "同事（IAS）：表示论与 Langlands 程序之交", "undir"),
    ("colleague", "Atle Selberg", "同事（IAS）：迹公式与表示论的交汇", "undir"),
    ("colleague", "Israel Gelfand", "同事（表示论领域）：Gelfand 与 Harish-Chandra 并行工作", "undir"),
    ("colleague", "Michael Atiyah", "同事（IAS）：Atiyah 对 Harish-Chandra 工作的评价", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-HarishChandra')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
