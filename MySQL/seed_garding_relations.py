#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Lars Gårding 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Lars_Gårding/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Gårding）
    ("advisor-student", "Marcel Riesz", "博士导师（隆德 1944）"),
    # 学生（有向：Gårding → 学生）
    ("advisor-student", "Lars Hörmander", "学生（与 Riesz 共同指导），Fields 1962"),
    ("advisor-student", "Vidar Thomée", "学生，数值分析"),
    ("advisor-student", "Bruce Kellogg", "学生"),
    ("advisor-student", "Per-Åke Wedin", "学生，数值分析"),
    ("advisor-student", "Gunnar Bergendal", "学生"),
    ("advisor-student", "Yang Liu", "学生"),
    # 合作者（无向）
    ("collaborator", "Michael Atiyah", "Petrovsky 空隙合作（1970 Acta）"),
    ("collaborator", "Raoul Bott", "Petrovsky 空隙合作（1970 Acta）"),
    ("colleague", "Ivan Petrovsky", "Petrovsky 空隙概念提出者"),
]

MARKER = "[Gårding-材料待展开] "

FIELDS = [
    ("partial differential equation", "偏微分方程", 0),
    ("differential operator", "微分算子", 1),
    ("mathematical physics", "数学物理", 2),
]

AWARDS = [
    ("Member of the Royal Swedish Academy of Sciences", "瑞典皇家科学院院士", 1953),
    ("Member of the Finnish Society of Sciences and Letters", "芬兰科学学会会员", 1985),
]

INSTITUTIONS = [
    ("Lund University", "education", 1937, 1944),
    ("Lund University", "employment", 1952, 1984),
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "拉尔斯·戈丁",
            '["Lars Gårding","Gårding 不等式","Gårding–Wightman 公理","瑞典 PDE 学派"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Gårding id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    # ---------- 5. 国籍 ----------
    cur.execute("SELECT id FROM countries WHERE name_en='Sweden'")
    se = cur.fetchone()
    if se:
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Marcel Riesz":
                f, t = pid, pid0  # 导师 → Gårding
            else:
                f, t = pid0, pid  # Gårding → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Gårding-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Lars Gårding' OR b.name_en='Lars Gårding'"""
    )
