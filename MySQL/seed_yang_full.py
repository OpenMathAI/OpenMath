#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Chen Ning Yang（杨振宁）人物主记录 + 研究领域 + 社会关系入库。

数据源：本地 Wikipedia (physicist/presentations/pages/20th_century/Chen_Ning_Yang)
     + 提示词 Chen_Ning_Yang_zh.md（第 4 / 4.5 步）。

关系方向约定：
  advisor-student：ADVISORS 中的人 → 杨振宁（导师）；其余 → 学生（杨振宁 → 学生）
...
# 导师（有向：→ 杨振宁）
ADVISORS = {"Edward Teller", "Enrico Fermi"}

# (relation_type, name_en, name_zh, note)
RELATIONS = [
    # 导师（→ 杨振宁）
    ("advisor-student", "Edward Teller", "爱德华·泰勒", "芝加哥大学博士导师，氢弹之父"),
    ("advisor-student", "Enrico Fermi", "恩里科·费米", "芝加哥学术顾问，1948–1949 任其助手"),
    # 门生（杨振宁 → 学生）
    ("advisor-student", "Bill Sutherland", "比尔·萨瑟兰", "博士生，可积系统"),
    ("advisor-student", "Alexander Wu Chao", "赵午", "博士生，加速器物理"),
    ("advisor-student", "Brosl Hasslacher", "布罗斯尔·哈斯拉彻", "博士生，孤子与场论"),
    # 合作者/同事（无向）
    ("colleague", "Robert Mills", "罗伯特·米尔斯", "1954 杨-米尔斯理论合作者"),
    ("colleague", "Tai Tsun Wu", "吴大峻", "Wu–Yang 磁单极子 / Wu–Yang 词典"),
    ("colleague", "Nina Byers", "妮娜·拜尔斯", "Byers–Yang 定理（磁通量子化）"),
    ("colleague", "Kerson Huang", "黄克孙", "Lee–Huang–Yang 弱相互作用玻色气体修正"),
    ("colleague", "Reinhard Oehme", "莱因哈德·厄姆", "1957 CPT 对称"),
    # 荣誉共同体（无向）
    ("co-honored", "Tsung-Dao Lee", "李政道", "1957 诺贝尔物理学奖共同得主"),
]

# 配偶（无向，占位 primary_occupation=NULL）
SPOUSES = [
    ("Chih-Li Tu", "杜致礼", "配偶（1950–2003）"),
    ("Weng Fan", "翁帆", "配偶（2005 至今）"),
]
...
def upsert_person(cur, name_en, name_zh, occupation, has_bio=0):
    """按 name_en 或 name_zh 查找，找不到则建占位。返回 pid。"""
...
def main():
    ...
    if row:
        ...
        cur.execute(
            ...
            (QID, "杨振宁", "male", "1922-10-01", "2025-10-18",
             "Chinese physicist (1922–2025)", "physicist", pid0),
        )
        ...
        cur.execute(
            ...
            (QID, NAME, "杨振宁", "male", "1922-10-01", "2025-10-18",
             "Chinese physicist (1922–2025)", "physicist"),
        )
        pid0 = cur.lastrowid
    ...
    def add_relation(rel, name_en, name_zh, note, occupation, is_advisor, is_student):
        ...
        cur.execute(
            ...
            "VALUES (%s,%s,%s,%s,'立传-Chen_Ning_Yang')",
            (f, t, rel, note),
        )
        if cur.rowcount:
            added += 1
    ...
    for rel, name_en, name_zh, note in RELATIONS:
        ...
        is_student = rel == "advisor-student" and not is_advisor
        add_relation(rel, name_en, name_zh, note, "physicist", is_advisor, is_student)

    for name_en, name_zh, note in SPOUSES:
        add_relation("spouse", name_en, name_zh, note, None, False, False)
