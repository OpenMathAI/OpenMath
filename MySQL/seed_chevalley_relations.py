#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Claude Chevalley 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Claude Chevalley"
RELATIONS = [
    ("advisor-student", "René Garnier", "博士导师", "in"),
    ("advisor-student", "Michel Broué", "博士生，表示论", "out"),
    ("advisor-student", "Gerhard Hochschild", "博士生，代数（Hochschild 同调）", "out"),
    ("advisor-student", "Léon Ehrenpreis", "博士生，偏微分方程（Ehrenpreis 定理）", "out"),
    ("colleague", "André Weil", "布尔巴基创始人之一：Chevalley 是 1934 年创始 6 人之一", "undir"),
    ("colleague", "Henri Cartan", "布尔巴基核心成员", "undir"),
    ("colleague", "Jean Dieudonné", "布尔巴基核心成员", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Chevalley')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
