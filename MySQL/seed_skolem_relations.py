#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Thoralf Skolem 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Thoralf_Skolem/（Wikipedia 存档）
"""
import re
import unicodedata

from db_mysql import get_conn

NAME = "Thoralf Skolem"
QID = "Q548080"

RELATIONS = [
    # 导师（有向：导师 → Skolem）
    ("advisor-student", "Axel Thue", "名义博士导师（1926 论文，Thue 已于 1922 年去世）"),
    # 学生（有向：Skolem → 学生）
    ("advisor-student", "Øystein Ore", "学生，后赴美国任教"),
    # 荣誉共同体（无向）
    ("co-honored", "Leopold Löwenheim", "Löwenheim–Skolem 定理共同命名（Löwenheim 1915 先证）"),
    ("co-honored", "Emmy Noether", "Skolem–Noether 定理（Skolem 1927 先证，Noether 独立再发现）"),
    # 同事/相关（无向）
    ("colleague", "David Hilbert", "原始递归算术成果被 Hilbert 学派占用/忽视"),
    ("colleague", "Kurt Gödel", "其不完备性设想（1929）后由 Gödel 正式证明（1931）"),
    ("colleague", "Kristian Birkeland", "物理学同事（1909 起任其助手）"),
    ("colleague", "Hao Wang", "逻辑学家，高度评价其工作"),
]

MARKER = "[Skolem-材料待展开] "

FIELDS = [
    ("mathematical logic", "数理逻辑", 0),
    ("model theory", "模型论", 1),
    ("set theory", "集合论", 2),
    ("abstract algebra", "抽象代数", 3),
    ("number theory", "数论", 4),
]

# 全部奖项收录
AWARDS = [
    ("Fridtjof Nansen Award of Excellence", "弗里乔夫·南森杰出奖", 0),
    ("Gunnerus Medal", "古纳鲁斯奖章", 0),
    ("Knight First Class of the Order of St. Olav", "圣奥拉夫勋章一等骑士", 0),
]

INSTITUTIONS = [
    ("University of Oslo", "education", 1905, None),
    ("University of Oslo", "employment", 1916, 1930),
    ("Chr. Michelsen Institute", "employment", 1930, 1938),
    ("University of Oslo", "professor", 1938, 1957),  # 同机构两段任职用不同 relation 区分
    ("University of Göttingen", "employment", None, None),
]


def norm(s: str) -> str:
    s = unicodedata.normalize("NFKD", s or "")
    s = "".join(c for c in s if not unicodedata.combining(c))
    return re.sub(r"[\s_\-\'\.\(\)·,]", "", s).lower()


def main():
    conn = get_conn()
    cur = conn.cursor()

    cur.execute("SELECT id, name_en, name_zh FROM people")
    people = [(pid, en, zh, norm(en or ""), norm(zh or "")) for pid, en, zh in cur.fetchall()]
    by_en = {ne: pid for pid, en, zh, ne, nz in people if ne}
    by_zh = {nz: pid for pid, en, zh, ne, nz in people if nz}

    cur.execute("SELECT id FROM occupations WHERE name_en='mathematician'")
    occ_id = cur.fetchone()[0]
    cur.execute("SELECT id FROM occupations WHERE name_en='philosopher'")
    phil_id = cur.fetchone()

    # ---------- 1. Skolem 本人补齐 ----------
    cur.execute("SELECT id FROM people WHERE name_en=%s", (NAME,))
    row = cur.fetchone()
    if not row:
        cur.execute(
            "INSERT INTO people(name_en, primary_occupation, has_biography, qid) "
            "VALUES (%s,'mathematician',0,%s)",
            (NAME, QID),
        )
        pid0 = cur.lastrowid
    else:
        pid0 = row[0]
    cur.execute(
        "UPDATE people SET qid=%s, name_zh=%s, name_variants=%s, description=%s, "
        "birth_date=%s, death_date=%s, has_social_data=1 WHERE id=%s",
        (
            QID,
            "托拉尔夫·斯科伦",
            '["Thoralf Albert Skolem","Löwenheim–Skolem 定理","Skolem 悖论","模型论先驱"]',
            "Norwegian mathematician and logician (1887–1963)",
            "1887-05-23",
            "1963-03-23",
            pid0,
        ),
    )
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    if phil_id:
        cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,1)",
                    (pid0, phil_id[0]))
    print(f"Skolem id={pid0} 已补齐 people 字段（has_social_data=1）")

    # ---------- 2. 研究领域 ----------
    for f_en, f_zh, rank in FIELDS:
        cur.execute("SELECT id FROM fields WHERE name_en=%s", (f_en,))
        frow = cur.fetchone()
        if not frow:
            cur.execute("INSERT INTO fields(name_en, name_zh) VALUES (%s,%s)", (f_en, f_zh))
            fid = cur.lastrowid
            print(f"  + 新建领域: {f_en} (id={fid})")
        else:
            fid = frow[0]
        cur.execute(
            "INSERT IGNORE INTO person_field(person_id, field_id, `rank`) VALUES (%s,%s,%s)",
            (pid0, fid, rank),
        )
    print("  领域关联完成")

    # ---------- 3. 奖项 ----------
    for a_en, a_zh, year in AWARDS:
        cur.execute("SELECT id FROM awards WHERE name_en=%s", (a_en,))
        arow = cur.fetchone()
        if not arow:
            cur.execute("INSERT INTO awards(name_en, name_zh, award_type) VALUES (%s,%s,'award')", (a_en, a_zh))
            aid = cur.lastrowid
            print(f"  + 新建奖项: {a_en} (id={aid})")
        else:
            aid = arow[0]
        note = None
        if year == 0:
            note = "年份待查"
        cur.execute(
            "INSERT IGNORE INTO award_laureate(person_id, award_id, `year`, share_type, source, note) "
            "VALUES (%s,%s,%s,'独享','Wikipedia',%s)",
            (pid0, aid, year, note),
        )
    print("  奖项关联完成")

    # ---------- 4. 机构 ----------
    for inst, rel, sy, ey in INSTITUTIONS:
        cur.execute("SELECT id FROM institutions WHERE name_en=%s", (inst,))
        irow = cur.fetchone()
        if not irow:
            cur.execute("INSERT INTO institutions(name_en) VALUES (%s)", (inst,))
            iid = cur.lastrowid
            print(f"  + 新建机构: {inst} (id={iid})")
        else:
            iid = irow[0]
        cur.execute(
            "SELECT 1 FROM person_institution WHERE person_id=%s AND inst_id=%s AND relation=%s",
            (pid0, iid, rel),
        )
        if not cur.fetchone():
            cur.execute(
                "INSERT IGNORE INTO person_institution(person_id, inst_id, relation, start_year, end_year) "
                "VALUES (%s,%s,%s,%s,%s)",
                (pid0, iid, rel, sy, ey),
            )
    print("  机构关联完成")

    # ---------- 5. 国籍 ----------
    cur.execute("SELECT id FROM countries WHERE name_en='Norway'")
    no = cur.fetchone()
    if no:
        cur.execute("INSERT IGNORE INTO person_nationality(person_id, country_id, `rank`) VALUES (%s,%s,0)",
                    (pid0, no[0]))
    print("  国籍关联完成")

    # ---------- 6. 社会关系 ----------
    created = 0
    relations_added = 0
    for rel, name, note in RELATIONS:
        pid = by_en.get(norm(name))
        if pid is None:
            pid = by_zh.get(norm(name))
        if pid is None:
            cur.execute(
                "INSERT INTO people(name_en, primary_occupation, has_biography) VALUES (%s,'mathematician',0)",
                (name,),
            )
            pid = cur.lastrowid
            cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                        (pid, occ_id))
            by_en[norm(name)] = pid
            created += 1
            print(f"  + 新建(占位): {name} (id={pid})")
        else:
            print(f"  已有: {name} (id={pid})")

        if rel == "advisor-student":
            if name == "Axel Thue":
                f, t = pid, pid0  # 导师 → Skolem
            else:
                f, t = pid0, pid  # Skolem → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Skolem-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1

    conn.commit()
    print(f"\n新建人物: {created} · 新增关系: {relations_added}")
    cur.execute("SELECT COUNT(*) FROM people")
    print(f"people 总数: {cur.fetchone()[0]}")

    print("\n=== 校验：Skolem 社会关系 ===")
    cur.execute(
        """SELECT a.name_en AS 甲, rt.name_zh AS 关系, b.name_en AS 乙, pr.note
        FROM person_relation pr
        JOIN people a ON a.id=pr.from_id
        JOIN people b ON b.id=pr.to_id
        JOIN relation_types rt ON rt.relation_key=pr.relation_type
        WHERE a.name_en='Thoralf Skolem' OR b.name_en='Thoralf Skolem'"""
    )
    for r in cur.fetchall():
        print(f"  {r[0]} {r[1]} {r[2]} — {(r[3] or '')[:40]}")

    conn.close()


if __name__ == "__main__":
    main()
