#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pierre Deligne 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Pierre Deligne"
RELATIONS = [
    ("advisor-student", "Alexander Grothendieck", "博士导师（Paris-Sud，1972）：概形理论继承人，证明 Weil 猜想", "in"),
    ("advisor-student", "Michael Rapoport", "博士生，算术几何", "out"),
    ("advisor-student", "Miles Reid", "博士生，代数几何", "out"),
    ("collaborator", "David Mumford", "合作者：Deligne–Mumford 叠（模空间理论）", "undir"),
    ("colleague", "Jean-Pierre Serre", "同事与导师之友：Serre 深度影响 Deligne", "undir"),
    ("colleague", "Alexander Beilinson", "同事（IAS）：动机理论", "undir"),
    ("colleague", "David Kazhdan", "同事（IAS）：表示论与代数几何", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Deligne')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
