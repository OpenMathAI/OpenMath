#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Rolf Nevanlinna 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Rolf Nevanlinna"
RELATIONS = [
    ("advisor-student", "Ernst Leonard Lindelöf", "博士导师（赫尔辛基，1919）", "in"),
    ("advisor-student", "Lars Ahlfors", "学生：首届菲尔兹奖得主，芬兰复分析学派双璧", "out"),
    ("advisor-student", "Olli Lehto", "博士生，拟共形映射（后为芬兰科学院院长）", "out"),
    ("advisor-student", "Kurt Strebel", "博士生，极值长度理论", "out"),
    ("advisor-student", "Kari Karhunen", "博士生，随机过程（Karhunen–Loève）", "out"),
    ("colleague", "Jacques Hadamard", "同事：值分布理论的对话者", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Nevanlinna')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
