#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""L.E.J. Brouwer 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "L.E.J. Brouwer"
RELATIONS = [
    ("advisor-student", "Diederik Korteweg", "博士导师（阿姆斯特丹，1907），数学物理", "in"),
    ("advisor-student", "Arend Heyting", "博士生，直觉主义逻辑（Brouwer–Heyting–Kolmogorov 解释）", "out"),
    ("colleague", "David Hilbert", "论战对手：Brouwer–Hilbert 之争（数学基础/直觉主义 vs 形式主义）", "undir"),
    ("colleague", "Hermann Weyl", "盟友：Weyl 一度支持直觉主义，后又动摇", "undir"),
    ("colleague", "John von Neumann", "批评者：von Neumann 是直觉主义的反对者", "undir"),
    ("colleague", "Felix Hausdorff", "同事（拓扑学同行）：同属点集拓扑奠基者", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Brouwer')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
