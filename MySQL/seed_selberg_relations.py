#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Atle Selberg 社会关系入库（第 4.5 步，§二十）。"""
import re
import unicodedata
...
ME = "Atle Selberg"
RELATIONS = [
    ("advisor-student", "Kai Man Tsang", "博士生，数论（曾俊文）", "out"),
    ("colleague", "Carl Ludwig Siegel", "同事（IAS）：Siegel 对 Selberg 影响深远", "undir"),
    ("colleague", "Harish-Chandra", "同事（IAS）：Selberg 迹公式与表示论的交汇", "undir"),
    ("colleague", "Robert Langlands", "同事（IAS）：Langlands 程序继承 Selberg 迹公式", "undir"),
    ("colleague", "André Weil", "同事（IAS 数论圈）", "undir"),
    ("collaborator", "Paul Erdős", "合作者：素数定理初等证明（1948，两人独立同时证明）", "undir"),
]
...
def main():
    ...
    for rel, name, note, d in RELATIONS:
        ...
        cur.execute("INSERT IGNORE INTO person_relation(from_id,to_id,relation_type,note,source) VALUES (%s,%s,%s,%s,'立传-Selberg')", (f, t, rel, MARKER + note))
        added += cur.rowcount
    conn.commit()
