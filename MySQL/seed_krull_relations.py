#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""梳理 Wolfgang Krull 的社会关系与研究领域，入库 greatminds 数据库。

数据源：mathematician/pages/Wolfgang_Krull/（Wikipedia 存档）
"""
import re
import unicodedata
...
RELATIONS = [
    # 导师（有向：导师 → Krull）
    ("advisor-student", "Alfred Loewy", "博士导师（哥廷根 1921）"),
    # 学生（有向：Krull → 学生）
    ("advisor-student", "Jürgen Neukirch", "学生，代数数论"),
    ("advisor-student", "Paulo Ribenboim", "学生，数论（编其文集）"),
    ("advisor-student", "Wilfried Brauer", "学生，计算机科学家（与 Richard Brauer 不同）"),
    ("advisor-student", "Karl-Otto Stöhr", "学生"),
    ("advisor-student", "Peter Dombrowski", "学生，微分几何"),
]

MARKER = "[Krull-材料待展开] "

FIELDS = [
    ("commutative algebra", "交换代数", 0),
    ("ring", "环论", 1),
    ("algebra", "代数学", 2),
]
...
INSTITUTIONS = [
    ...
    ("University of Rostock", "education", None, None),
    ("University of Göttingen", "education", 1919, 1921),
    ("University of Freiburg", "employment", None, None),
    ("University of Erlangen-Nuremberg", "employment", None, None),
    ("University of Bonn", "employment", 1939, 1971),
]
...
def main():
    ...
    cur.execute(
        ...
        (
            QID,
            "沃尔夫冈·克鲁尔",
            '["Wolfgang Krull","Krull 维数","Krull 主理想定理","交换代数奠基人"]',
    ...
    cur.execute("INSERT IGNORE INTO person_occupation(person_id, occupation_id, `rank`) VALUES (%s,%s,0)",
                (pid0, occ_id))
    print(f"Krull id={pid0} 已补齐 people 字段（has_social_data=1）")
    ...
    for rel, name, note in RELATIONS:
        ...
        if rel == "advisor-student":
            if name == "Alfred Loewy":
                f, t = pid, pid0  # 导师 → Krull
            else:
                f, t = pid0, pid  # Krull → 学生
        else:
            f, t = sorted([pid0, pid])

        cur.execute(
            "INSERT IGNORE INTO person_relation(from_id, to_id, relation_type, note, source) "
            "VALUES (%s,%s,%s,%s,'Krull-presentation')",
            (f, t, rel, MARKER + note),
        )
        if cur.rowcount:
            relations_added += 1
    ...
    cur.execute(
        ...
        WHERE a.name_en='Wolfgang Krull' OR b.name_en='Wolfgang Krull'"""
    )
